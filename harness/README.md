...
| [`make mutate`](sensors/mutate.md) | Mutations-Sensor: färbt jede kuratierte Mutation ihren Wächter rot? `MUTATE_CASES` fährt nur die genannten Fälle, ohne den Beleg-Slot zu berühren | kein Gate · [`AGENTS.md`](../AGENTS.md) §3.6 |
...
| `make tap-check` | hält die Formel am Kopf des Default-Branch des Tap byte-genau gegen das Asset des Tags (`TAG=<tag>`): Exit des Skripts 0 gleich oder Vorab-Tag, 1 Formel-Unterschied auch nach dem zweiten Lesen, 2 nicht ausführbar — `make` selbst endet bei jedem Fehlschlag mit 2, und die Klasse des Skripts steht in der letzten stderr-Zeile **des Skripts** `tap-check: Exit <N>` (bei Exit 0 fehlt sie; sie ist der Vertrag, die Ziffer in der Meldung von `make` nennt die Klasse ebenfalls, ihr Wortlaut hängt an der Locale; bei `make tap-check` aus dem Wurzelverzeichnis ist die Zeile die vorletzte der Ausgabe, unter `-C` oder einem umschließenden `make` folgen weitere Zeilen; nicht zugesagt bei Signal — dort fehlt auch die Klasse, der Prozess-Exit ist 128 plus die Signalnummer, wenn das Signal das Skript im Direktaufruf oder `make` selbst trifft, und 2, wenn es das Skript unter `make` trifft —, nicht beschreibbarer stderr — dort ist auch die Klasse nicht zugesagt, ein Formel-Unterschied kann als Klasse 2 enden — und fehlendem oder unbekanntem Modus); lesend, Transport im gepinnten Bild, braucht Netz an genau diesem Aufruf | kein Gate · [`ADR-0064`](../docs/plan/adr/0064-tap-nachzug-ein-skript-zwei-aufrufer-byte-kontrolle-gegen-das-asset.md) |
| `make tap-nachzug` | zieht die Formel des Tags (`TAG=<tag>`) ins Tap nach: schreibt die Bytes des veröffentlichten Assets in die Formel-Datei am Kopf des Default-Branch des Tap — nur vorwärts (der Tag-Kern ist nicht kleiner als der Stand der `version`-Zeile am Tap-Kopf), idempotent (gleiche Bytes: kein Schreibaufruf), optimistisch gegen den gelesenen Blob-Stand — und kontrolliert danach wie `make tap-check`; **schreibend, braucht `TAP_TOKEN` in der Umgebung des Aufrufers** (ohne ihn Exit 2 vor jedem Netz-Zugriff; das Token steht nie in einer Kommandozeile und nie in der Ausgabe). Exit des Skripts: 0 gleich, nachgezogen oder Vorab-Tag, 1 Formel-Unterschied in der Nachkontrolle, 2 nicht ausführbar (Vorwärts-Schutz, ausdrückliche Ablehnung mit *„Tap unverändert"* und *„Ausgang ungewiss"* eingeschlossen) — `make` selbst endet bei jedem Fehlschlag mit 2, und die Klasse des Skripts steht in der letzten stderr-Zeile **des Skripts** `tap-sync: Exit <N>` (bei Exit 0 fehlt sie; bei `make tap-nachzug` aus dem Wurzelverzeichnis ist sie die vorletzte der Ausgabe; nicht zugesagt bei Signal — dort fehlt auch die Klasse —, nicht beschreibbarer stderr und fehlendem oder unbekanntem Modus); Transport im gepinnten Bild, braucht Netz an genau diesem Aufruf; der Schreib-Pfad ist gegen eine nachgebildete Schnittstelle geprüft, am realen Tap belegt ihn erst ein realer Nachzug | kein Gate · [`ADR-0064`](../docs/plan/adr/0064-tap-nachzug-ein-skript-zwei-aufrufer-byte-kontrolle-gegen-das-asset.md), [`ADR-0066`](../docs/plan/adr/0066-exit-klassen-des-tap-werkzeugs-sind-die-des-skripts.md) |
