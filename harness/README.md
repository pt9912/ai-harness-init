...
Der **Träger** ist ausschließlich der PreToolUse-Zusatz-Hook [`.claude/hooks/pretooluse-commit-msg-guard.sh`](../.claude/hooks/pretooluse-commit-msg-guard.sh), zweiter Eintrag im `"Bash"`-Matcher neben `pretooluse-command-guard.sh` ([`.claude/settings.json`](../.claude/settings.json)): er spiegelt jeden `git commit … -F <datei> …`-Aufruf (Repo-Konvention „Commit via Message-Datei") **vor** der Ausführung nach `make commit-msg-check MSG=<datei>` und blockt bei Exit ≠ 0 — der Commit steht dann nicht. Erkannt wird **nur** diese eine Aufrufform; `git commit -m "…"` und `-F -` (stdin) entkommen der Prüfung — derselbe Stolperdraht-Charakter wie beim Nachbar-Guard, kein Sandbox-Anspruch ([ADR-0004](../docs/plan/adr/0004-durchsetzungs-emission.md)). Fehlt die Message-Datei (noch), greift der Hook nicht — kein Rateversuch. Die Prüf-Instanz ist über `PRETOOLUSE_COMMIT_MSG_CHECKER` austauschbar (Default: der `make`-Aufruf oben); einziger Grund ist Testbarkeit — das gepinnte `BATS_IMAGE` fährt ohne Docker und ohne `git` (dieselbe Grenze wie bei `slice-mv`/`archive-welle`), `test/commit-msg-guard.bats` hält Match-Extraktion, Datei-Existenz-Vorprüfung und beide Verdikte der (gestubbten) Prüf-Instanz hermetisch; der reale `docker`/`make`-Lauf gegen eine echte Datei steht oben in diesem Absatz, nicht im bats-Fall. `test/mutations/307-commit-msg-guard-block-bedingung-invertiert.sh` nimmt der Block-Bedingung die Zähne (Kennungs-Prüfung wirkungslos) und muss `test/commit-msg-guard.bats` röten.

**Der Prüfbereich trägt seinen Cutoff, und er ist rein prospektiv.** Ein Maßstab über die ganze Historie wäre an einem Bestand rot, den niemand mehr ändern kann ([`LH-QA-01`](../spec/lastenheft.md#lh-qa-01--keine-halluzinierten-gates-f4-f5-f6), dieselbe Cutoff-Begründung wie [`MR-015`](conventions.md#mr-015--change-request-bei-personalunion-von-auftraggeber-und-entwickler)): gemessen (`git log --format='%s' | grep -vcE 'ADR-[0-9]{4}|LH-[A-Z]{2}-[0-9]{2}|MR-[0-9]{3}|slice-[0-9]+|^(Merge |Revert )'`) trugen **239 von 2098** Commit-Betreffs der gesamten Historie keine Kennung, in den letzten 50 noch **3** (in den letzten 20 **0**) — ein rückwirkender Sensor wäre am Tag seiner Aktivierung tief rot. Der PreToolUse-Hook prüft strukturell nur den **werdenden** Commit, nie die Historie — er braucht deshalb keinen Cutoff-Zeitpunkt: Der Cutoff **ist** „ab dem ersten Aufruf dieses Hooks", nicht ein Datum in der Konfiguration. `doc-commits` (`d-check.mk`, `--range`) bleibt darum advisory und ungenutzt für diese Zusage — nicht aus Vorsicht, sondern gemessen defekt: Der gepinnte `v0.74.1` bricht `--range`-Läufe des `commits`-Moduls mit `Range-Basis-Vorfahren nicht lesbar: object not found` ab, sobald `commits.id-patterns` eine **eigene** Liste trägt (von den drei eingebauten Vorschlägen `ADR-\d{4}`/`DC-(FA-[A-Z]+|QA)-\d+`/`slice-\d+` abweichend) — reproduziert unabhängig von Range-Tiefe (`HEAD~1..HEAD` bricht ebenso wie `HEAD~20..HEAD`), von expliziten Commit-Hashes statt `HEAD~N`, und von den `--disable`-Flags; verschwindet mit einer **leeren** `id-patterns`-Liste (dann laufen die drei eingebauten Muster, ohne `LH-*`/`MR-*` zu treffen) und mit `--commit-msg` statt `--range` (dort funktioniert dieselbe eigene Liste fehlerfrei, s. o.). `--enable vcs`/`doc-immutable` mit demselben `commits:`-Block im Baum bleibt unberührt (isoliert an einer Kopie außerhalb des Repos, netzlos, Digest aus `d-check.mk`). Ein Range-Lauf in CI ist damit **nicht** in diesem Slice entschieden — er bräuchte entweder einen Tool-Fix oder den Verzicht auf `LH-*`/`MR-*` in `id-patterns`, beides eine eigene Abwägung.
