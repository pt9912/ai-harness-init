...
| Commit aus einem Repo-Werkzeug (`make slice-mv`, `archive-welle` committen intern) | strukturell nicht erreicht — der Kanal sieht `make slice-mv …` | erreicht, sobald die Werkzeug-Message eine Kennung trägt — die Messages der benannten Slices ([`MR-057`](conventions.md#mr-057--die-kennungs-form-für-neue-slices-und-wellen-ist-der-name-nicht-die-nummer)) und von `archive-welle` tragen ihre eigene Slice- bzw. Welle-Kennung bereits (`$base` bzw. `b.Welle`); die Kennungs-Menge aus `commits.id-patterns` erkennt seit `slice-lifecycle-werkzeuge-tragen-die-kennung` neben der Nummernform auch den freien Slug für beide Klassen (`slice-<slug>`, `welle-<slug>`), sodass ihr Commit den Träger erreicht statt an ihm zu brechen: `git log --format='%s' \| grep '^slice-mv:' \| grep -vcE 'ADR-[0-9]{4}\|LH-[A-Z]{2}-[0-9]{2}\|MR-[0-9]{3}\|slice-[0-9]+\|slice-[a-z][a-z0-9]*(-[a-z0-9]+)+\|welle-[0-9]+\|welle-[a-z][a-z0-9]*(-[a-z0-9]+)+'` → **1 von 594** (kein Erwartungswert, [`MR-025`](conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert) Setzung 2; der eine Rest ist eine Message aus einer älteren Werkzeug-Fassung ohne eingebettete Kennung, kein Naming-Fall) |
