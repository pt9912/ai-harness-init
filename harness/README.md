...
**Was `comment-claims` nicht deckt — benannt, weil eine Vollständigkeits-Zeile („N Datei(en) geprueft, 0 Befund(e)") sonst mehr behauptet als sie trägt** (Review-Befund HIGH-1 vom 2026-07-30): der Prüfbereich entsteht im Rezept aus `git ls-files` und ist an **zwei** Stellen enger als der Gate-Stempel, den `record-gates` über den Arbeitsbaum legt. (1) Er kennt nur den **Index**: eine neu angelegte, noch **untrackte** Datei liegt innerhalb des bestätigten Baum-Zustands und außerhalb des Prüfbereichs — sie wird erst nach ihrem ersten `git add` geprüft. (2) Test-Dateien sind ausgenommen (ein Kommentar dort behauptet keine Abdeckung, sondern *ist* eine). Wer eine Datei neu anlegt und ihre Zusagen gedeckt sehen will, lässt den Gate **nach** dem `git add` laufen; ein Lauf davor ist eine Aussage über die anderen Dateien. Der Mechanismus selbst ist hier **nicht** geändert (Gate-*Anheben* ist ein Steering-Loop nach [`MR-001`](conventions.md#mr-001--doc-gate-schärfung-matrix--link-pflicht--anker-ids), und er betrifft jede künftige neue Datei, nicht die Telemetrie).
