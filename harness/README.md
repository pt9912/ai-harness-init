...
| `make tap-check` | hält die Formel am Kopf des Default-Branch des Tap byte-genau gegen das Asset des Tags (`TAG=<tag>`): Exit des Skripts 0 gleich oder Vorab-Tag, 1 Formel-Unterschied auch nach dem zweiten Lesen, 2 nicht ausführbar — `make` selbst endet bei jedem Fehlschlag mit 2 und nennt den Exit des Skripts in seiner Zeile `Fehler <N>`; lesend, Transport im gepinnten Bild, braucht Netz an genau diesem Aufruf | kein Gate · [`ADR-0064`](../docs/plan/adr/0064-tap-nachzug-ein-skript-zwei-aufrufer-byte-kontrolle-gegen-das-asset.md) |
