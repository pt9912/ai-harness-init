...
| `make e2e-abdeckung` | erzeugt [`docs/user/e2e-abdeckung.md`](../docs/user/e2e-abdeckung.md) aus den Stufen-Deklarationen in `harness/tools/full-smoke.sh` — liest Text, fährt keinen E2E | kein Gate; den Inhalt der erzeugten Datei hält ein Fall in [`test/e2e-abdeckung.bats`](../test/e2e-abdeckung.bats) (läuft in `make test`) · [`LH-QA-01`](../spec/lastenheft.md#lh-qa-01--keine-halluzinierten-gates-f4-f5-f6) |
