...
>
> **ÜBERHOLT: in §Grenze der erste Punkt „Liegen Objekte der Range in einem Pack, dessen Name nicht mit `pack-` beginnt, brechen die history-lesenden Ziele ab …“ als Aussage über den gepinnten Stand → [`MR-066`](../conventions.md#mr-066--d-check-pin-v0763-packs-unter-fremdem-präfix-lesbar-range-immer-aufgelöst).** Für `v0.76.1` gemessen gilt er dort fort, ebenso Pin, Digest, Strenge-Bilanz und die übrigen Messungen; unter dem gepinnten `v0.76.3` liest d-check ein solches Pack und prüft die Range. Der Auflösungs-Trigger dieses Eintrags ist damit eingelöst.
