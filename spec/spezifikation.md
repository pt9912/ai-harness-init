...
- **Abschnittsnummern werden nie neu vergeben.** Sie sind die der vendored Vorlage
  `.harness/baseline/v6.0.0/templates/spec/spezifikation.template.md`; ein
...
fliegt raus"*,
[Modul 15 §Span-/Audit-Attribut-Regeln](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#span-audit-attribut-regeln)).
...
| `SPEC-010` | `agent_role` | Pflicht | *Welche Rolle verursachte den Zugriff?* — das Pflichtfeld aus [Modul 15](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#span-audit-attribut-regeln). Gefüllt, wenn `agent_type` eine Harness-Rolle **nennt** (`planner`, `architect`, `implementer`, `reviewer`, `verifier`, `validator`). **Leer heißt UNBEKANNT, nie „rollenlos"** — s. die Lesevorschrift unten | — |
...
| `SPEC-013` | `adr` | Pflicht | *Auf wessen Entscheidung lief der Zugriff?* — die dritte Korrelations-Achse aus [Modul 15 §Kernidee](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#kernidee-modul-15), aus demselben `Bezug:`-Block wie `requirement`, Liste | — |
| `SPEC-014` | `branch`, `commit` | Pflicht | *Zu welcher Änderung gehört der Zugriff?* — die dritte Korrelations-Achse aus [Modul 15](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#span-audit-attribut-regeln) (*Slice/**PR**/Agent-Rolle*), abgeleitet aus `.git/HEAD`; die PR-Nummer selbst ist nicht erreichbar, s. Abweichung 2 | `internal/span/span_test.go` (Ableitung von `branch`) · Fall 111 |
...
| `SPEC-028` | `model_version` | Optional | *Welches Modell verursachte die Kosten?* — das Label `model.version` aus [Modul 15 §Cache-Counter-Regeln](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#cache-counter-regeln), aus `tool_response.resolvedModel`, **strukturell begrenzt** (Länge und geschlossener Zeichensatz, [§3](#3-defaults-und-konstanten)). Was die Gestalt eines Bezeichners nicht hat, wird **verworfen, nicht gekürzt** | `TestResolvedModelIsStructurallyBounded` · Fall 129 |
...
- Vom **Pflicht-Minimum** eines Audit-Span-Schemas — Slice-ID · Agent-Rolle · Cache-Status ·
  `requirement.id`
  ([§Span-/Audit-Attribut-Regeln](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#span-audit-attribut-regeln))
...
- **2** (PR-Nummer) weicht von den **Mindestfeldern eines Tool-Call-Spans** ab
  ...
  [Regelgruppe](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#span-audit-attribut-regeln),
  eine andere Liste).
- **5** (Hintergrund-Lauf ohne Verbrauchs-Achse) und **6** (Haupt-Kontext ohne Zahl) weichen von
  den
  [**Token-Attributions-Regeln**](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#token-attributions-regeln)
  ab.
...
1. **Cache-Status ist unerreichbar — für den Haupt-Kontext dauerhaft, für Subagenten-Läufe,
   ...
   [Modul 15 §Cache-Counter-Regeln](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#cache-counter-regeln)
...
2. **Die PR-NUMMER steht nicht im Span, ihr Anker schon.**
   [Modul 15](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#span-audit-attribut-regeln)
...
3. **`agent_role` ist heute durchweg leer, und das ist der Befund — nicht das Feld.** Die
   ...
   **Die kanonischen Namen der Agenten-Typen:** `planner` · `architect` · `implementer` ·
   ...
   [Modul 8](../.harness/baseline/v6.0.0/regelwerk/modul-08-agentenrollen.md#rollen-sequenz-für-einen-slice),
   ...
   **Lesevorschrift, bindend für jede Auswertung:** eine Rolle gibt es **immer** — jeder
   ...
   [Modul 15 §Token-Attributions-Regeln](../.harness/baseline/v6.0.0/regelwerk/modul-15-observability.md#token-attributions-regeln)
