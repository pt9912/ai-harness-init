...
`--print-mk`-Fragment, a-check-Image per Digest gepinnt). a-check prüft die hexSlice-Schichten
(`domain`/`application`/`ports`/`adapters`, inward-only) im emittierten Sprachskelett — read-only,
netzlos. Der Prüfbereich entsteht über die **Architektur-Achse** ([`LH-FA-04`](../spec/lastenheft.md#lh-fa-04--sprachskelett-picker-f4)):
das Gate wird **genau dann** emittiert, wenn ein schichten-tragendes Layout (`--arch hexslice`)
gewählt ist. **Vorbedingung erfüllt:** a-check ist — wie d-check — als gepinntes Image mit
`--print-mk` real verfügbar (v0.15.0, Digest-gepinnt); die Umsetzung stützt sich auf diesen Beleg.

**Akzeptanzkriterien:**

- **Happy Path:** Given `add-lang <sprache> <pfad> --arch hexslice` (das Skelett trägt
  `domain`/`application`/`ports`/`adapters`), then `.a-check.yml` + `a-check.mk` liegen im Zielrepo und
