...
- **Arch-Achse:** Given `add-lang go <pfad> --arch hexslice`, then das Skelett trägt die hexSlice-Schichten (`domain` / `application` / `ports` / `adapters` + Composition Root `cmd/`, die Bau-Gerüstung unverändert); **ohne** `--arch` (bzw. `--arch flat`) bleibt das flache Skelett **byte-identisch** ([`LH-QA-02`](../spec/lastenheft.md#lh-qa-02--reproduzierbarkeit)).
