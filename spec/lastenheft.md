...
Default **`flat`** (das heutige Verhalten), opt-in **`hexslice`** (HexSlice = Hexagonal +
Vertical Slice: `domain` / `application` (Use-Case-Slices) / `ports` / `adapters`).
...
**Unterstützte Architekturen:** `flat` (Default), `hexslice` (weitere nur mit
