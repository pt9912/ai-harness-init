...
![hooks-lifecycle](images/hooks-lifecycle.png)
...
![hook-resolution](images/hook-resolution.png)
...
| `FileChanged`                                                                                                                                     | Dateinamen zum Überwachen (siehe [#ERROR!](#filechanged)) | `.envrc\|.env`                                                                                                                                                                      |
...
Handler werden im aktuellen Verzeichnis mit der Umgebung von Claude Code ausgeführt. Die Umgebungsvariable `$CLAUDE_CODE_REMOTE` wird in Remote-Web-Umgebungen auf `"true"` gesetzt und ist in der lokalen CLI nicht gesetzt. {/* min-version: 2.1.199 */}Ab v2.1.199 wird [`$CLAUDE_CODE_BRIDGE_SESSION_ID`](/docs/de/env-vars) auf die [Remote Control](/docs/de/remote-control) Sitzungs-ID gesetzt, während die lokale Sitzung eine aktive Remote Control-Verbindung hat.
...
| Feld          | Erforderlich | Beschreibung                                                                                                                                                                                                                                                                                                                                                                                     |
| :------------ | :----------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `command`     | ja           | Shell-Befehl zum Ausführen. Mit `args` die ausführbare Datei zum direkten Spawnen. Siehe [#NAME?](#exec-form-and-shell-form)                                                                                                                                                                                                                                                                     |
| `args`        | nein         | Argumentliste. Wenn vorhanden, wird `command` als ausführbare Datei aufgelöst und direkt mit `args` als Argumentvektor gespawnt, ohne Shell. Siehe [#NAME?](#exec-form-and-shell-form)                                                                                                                                                                                                           |
| `async`       | nein         | Wenn `true`, wird im Hintergrund ausgeführt, ohne zu blockieren. Siehe [#NAME?](#run-hooks-in-the-background)                                                                                                                                                                                                                                                                                    |
...
| `input`  | nein         | Argumente, die an das Tool übergeben werden. String-Werte unterstützen `${path}`-Substitution aus der [#NAME?](#hook-input-and-output) des Hooks, wie `"${tool_input.file_path}"`                                                                                                                                                                      |
...
| `agent_type` | Agent-Name (zum Beispiel `"Explore"` oder `"security-reviewer"`). Vorhanden, wenn die Sitzung `--agent` verwendet oder der Hook innerhalb eines Subagenten ausgelöst wird. Für Subagenten hat der Typ des Subagenten Vorrang vor dem `--agent`-Wert der Sitzung. Für [#NAME?](#subagentstart), um zu erfahren, wie Sie einen Matcher gegen einen Plugin-bezogenen Namen schreiben. |
...
| :---------------------------------------------------------------------------------------------------------------------------------- | :------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| UserPromptSubmit, UserPromptExpansion, PostToolUse, PostToolUseFailure, PostToolBatch, Stop, SubagentStop, ConfigChange, PreCompact | Top-Level `decision`             | `decision: "block"`, `reason`. Stop und SubagentStop akzeptieren auch `hookSpecificOutput.additionalContext` für [#NAME?](#stop-decision-control)                                          |
...
| SessionStart, Setup, SubagentStart                                                                                                  | Nur Kontext                      | `hookSpecificOutput.additionalContext` fügt Kontext für Claude hinzu. SessionStart akzeptiert auch [#NAME?](#sessionstart-decision-control). Keine Blockierung oder Entscheidungskontrolle |
...
| Feld                 | Beschreibung                                                                                                                                                                                                                                                                                                                                                                                           |
| :------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `additionalContext`  | Zeichenkette, die zu Claudes Kontext am Anfang des Gesprächs hinzugefügt wird, vor dem ersten Prompt. Siehe [#NAME?](#add-context-for-claude), um zu erfahren, wie der Text bereitgestellt wird und was Sie darin einfügen sollten                                                                                                                                                                     |
...
| `watchPaths`         | Array von absoluten Pfaden zum Überwachen von [#NAME?](#filechanged)-Ereignissen während dieser Sitzung                                                                                                                                                                                                                                                                                                |
...
| `additionalContext`      | Zeichenkette, die zu Claudes Kontext hinzugefügt wird, zusammen mit dem eingereichten Prompt. Siehe [#NAME?](#add-context-for-claude)             |
...
| `additionalContext` | Zeichenkette, die zu Claudes Kontext zusammen mit dem erweiterten Prompt hinzugefügt wird. Siehe [#NAME?](#add-context-for-claude) |
...
| Feld                | Typ          | Beispiel                                              | Beschreibung                                                                                                                                                                                                                                                            |
| :------------------ | :----------- | :---------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `status`            | Zeichenkette | `"completed"`                                         | `"completed"` für Vordergrund-Subagenten, `"async_launched"` für Hintergrund-Subagenten. {/* min-version: 2.1.198 */}Ab v2.1.198 werden Subagenten standardmäßig im Hintergrund ausgeführt, daher erzeugt ein weggelassenes `run_in_background` auch `"async_launched"` |
...
| `resolvedModel`     | Zeichenkette | `"claude-sonnet-4-5"`                                 | Modell, auf dem der Subagent ausgeführt wurde, das sich vom angeforderten Modell unterscheiden kann. {/* min-version: 2.1.174 */}Erfordert Claude Code v2.1.174 oder später                                                                                             |
...
| `additionalContext`        | Zeichenkette, die zu Claudes Kontext vor der Tool-Ausführung hinzugefügt wird. Für `"defer"`, ignoriert. Siehe [#NAME?](#add-context-for-claude)                                                                                                                                                                                                                                                                                                                                                                           |
...
| `updatedPermissions` | Nur für `"allow"`: Array von [#NAME?](#permission-update-entries) zum Anwenden, wie das Hinzufügen einer Allow-Regel oder das Ändern des Session-Berechtigungsmodus                                                                                         |
...
| `setMode`           | `mode`, `destination`              | Ändert den Berechtigungsmodus. Gültige Modi sind `default`, `auto`, `acceptEdits`, `dontAsk`, `bypassPermissions`, `plan` und {/* min-version: 2.1.200 */}`manual` als Alias für `default`. Der Alias `manual` erfordert Claude Code v2.1.200 oder später |
...
| `additionalContext`    | Zeichenkette, die zu Claudes Kontext zusammen mit dem Tool-Ergebnis hinzugefügt wird. Siehe [#NAME?](#add-context-for-claude)                                       |
...
| `updatedMCPToolOutput` | Ersetzt die Ausgabe nur für [#NAME?](#match-mcp-tools). Bevorzugen Sie `updatedToolOutput`, das für alle Tools funktioniert                                         |
...
| Feld                | Beschreibung                                                                                                           |
| :------------------ | :--------------------------------------------------------------------------------------------------------------------- |
| `additionalContext` | Zeichenkette, die zu Claudes Kontext zusammen mit dem Fehler hinzugefügt wird. Siehe [#NAME?](#add-context-for-claude) |
...
| Feld                | Beschreibung                                                                                                                                                                                                                        |
| :------------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `additionalContext` | Kontext-Zeichenkette, die einmal vor dem nächsten Modell-Aufruf eingefügt wird. Siehe [#NAME?](#add-context-for-claude) für Lieferdetails, was Sie darin einfügen sollten und wie fortgesetzte Sitzungen vergangene Werte handhaben |
...
| Feld                | Beschreibung                                                                                                                                                    |
| :------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `additionalContext` | Zeichenkette, die zu Claudes Kontext am Anfang des Gesprächs des Subagenten hinzugefügt wird, vor seinem ersten Prompt. Siehe [#NAME?](#add-context-for-claude) |
...
| `continueOnBlock` | nein         | Wenn der Prompt `ok: false` zurückgibt, wird der Grund an Claude zurückgegeben und der Turn wird fortgesetzt, anstatt zu stoppen. Standard: `false`. Implementiert als `continue: true` auf der resultierenden `decision: "block"`. Siehe [#NAME?](#response-schema) für ereignisspezifisches Verhalten |
