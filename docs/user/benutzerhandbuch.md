...
|---|---|---|
| **kanonisch** | das Regelwerk samt Vorlagen unter `.harness/baseline/`; die Skills unter `.harness/skills/`; `Makefile`, `d-check.mk` (bei einer geschichteten Bauform auch `a-check.mk`) und die `.mk`-Bausteine unter `harness/mk/`; die Skripte unter `tools/harness/`; `.claude/settings.json` und die Skripte unter `.claude/hooks/`; `harness/erfassung-feldliste.md`, `.harness/.gitignore` sowie die `.gitattributes` unter `.harness/` und `tools/harness/` | wird jedes Mal auf den Soll-Stand neu geschrieben, den dieses Programm mitbringt — eine von Hand geänderte Datei ist danach wieder die mitgelieferte <!-- d-check:ignore (die Pfade entstehen erst im aufgesetzten Repository) --> |
| **nur bei fehlender Datei** | **jede andere** Datei, die das Aufsetzen anlegt — die Prüf-Konfiguration `.d-check.yml` (bei einer geschichteten Bauform auch `.a-check.yml`), `.golangci.yml`, das `Dockerfile`, die Rollen-Anweisungen unter `.claude/agents/` und `.claude/commands/`, `.githooks/commit-msg` und die drei `.gitattributes` unter `harness/mk/`, `.claude/hooks/` und `.githooks/`; dazu **Ihre gefüllten Dateien** — die Dokumente unter `spec/` und `docs/plan/planning/`, `README.md`, `AGENTS.md`, `harness/README.md`, `harness/conventions.md`, Ihr Quellcode im Grundgerüst (`go.mod`, `cmd/app/main.go` …) | wird nur an einem freien Pfad geschrieben; eine vorhandene Datei bleibt **unangetastet** <!-- d-check:ignore (die Pfade entstehen erst im aufgesetzten Repository) --> |

Bei `.githooks/commit-msg` und den drei `.gitattributes` nennt der Lauf zusätzlich die Datei und die Folge (siehe [Zeilenenden und Kennungs-Form der Prüf-Konfiguration](#zeilenenden-und-kennungs-form-der-prüf-konfiguration)); für die übrigen Dateien der zweiten Klasse bleibt er still. Die zweite Klasse ist damit durch die erste bestimmt: was dort nicht steht, wird nur bei fehlender Datei geschrieben.
...
**Zeilenenden.** Das Aufsetzen legt in fünf Verzeichnissen je eine `.gitattributes` mit der einen Zeile `* text=auto eol=lf` an — dort, wo ein Programm die Dateien ausführt oder prüft (Shell-Skripte, `make`-Bausteine, Hooks; dazu die Prüfsummen des Regelwerks, die `make baseline-verify` hält) und ein CR am Zeilenende sie bricht. Die Zeile legt LF fest, unabhängig von `core.autocrlf` und von einer `.gitattributes` in der Wurzel Ihres Repositorys; ein Klon mit `core.autocrlf=true` trägt in diesen Verzeichnissen kein CR, solange die Dateien mit LF im Repository liegen — so legt das Werkzeug sie an. Die Wurzel bekommt keine `.gitattributes`. Dort liegen neben Ihren eigenen Dateien zwei Werkzeug-Dateien, `Makefile` und `d-check.mk`: sie liegen mit LF im Repository, ein Klon mit `core.autocrlf=true` schreibt sie nach Ihrer eigenen Einstellung mit CRLF, und ob ein `make` unter Windows daran scheitert, ist nicht gemessen. Die fünf Dateien des Werkzeugs zählt, im aufgesetzten Repository ausgeführt:

```bash
find . -name .gitattributes -not -path './.harness/baseline/*' | wc -l    # 5
```

Das Kommando zählt **jede** `.gitattributes` außerhalb von `.harness/baseline/`. Trägt Ihr Repository weitere — etwa eine eigene in der Wurzel —, ist die Zahl größer (mit einer in der Wurzel: 6); die fünf des Werkzeugs sind die in den fünf Verzeichnissen der folgenden Tabelle.
...
| `harness/mk/`, `.claude/hooks/`, `.githooks/` | eine vorhandene Datei bleibt unberührt; der Lauf nennt Datei und Folge, hier für `harness/mk/` (für die beiden anderen steht ihr Verzeichnis an dieser Stelle) <!-- d-check:ignore (die Pfade entstehen erst im aufgesetzten Repository) --> |

```text
ai-harness-init: harness/mk/.gitattributes liegt bereits — die Datei bleibt unberuehrt (skip-if-present). Traegt sie die Zeile `* text=auto eol=lf` nicht, tragen die Dateien in harness/mk/ im Klon mit core.autocrlf=true CRLF.
```

Die Meldung für `.githooks/commit-msg` lautet anders: sie nennt die mitgelieferte Prüfung `tools/harness/commit-msg-traceability.sh`, die daneben bereitliegt.
