...
die Feldtabelle nicht in der ADR steht; ihre Folgepflicht 1 nennt dafür einen Zielort, den
diese ADR ersetzt)

**Revidiert (Teil-Supersede):** die **Zielort-Setzung aus Folgepflicht 1** von
[ADR-0011](0011-telemetrie-erfassung-policy.md) — *„das Span-Schema … wird als `MR-<NNN>` in
`harness/conventions.md` geführt"*. Die **Begründung** dieser Folgepflicht gilt unverändert
fort (*„es ist eine Strukturregel, kein Implementierungsdetail, und der nächste Leser muss es
ohne Code finden"*), und mit ihr **jede** Setzung von
[ADR-0011](0011-telemetrie-erfassung-policy.md): die sechs Festlegungen, die Fitness Function
und die Folgepflichten 2–5 bleiben unberührt und **Accepted**. Weil
...
revidierte Zielort-Setzung als aktiv zitieren.
...
Aufwärts-Deklaration: wer diese ADR ändert, zieht diese beiden Abschnitte nach.
...
**Wohin die technischen Festlegungen stattdessen liefen** (gemessen 2026-08-01, jede Zahl über
die Blockgrenzen `### MR-` → `## Modus-Deklaration` am letzten committeten Stand): der
Adaptions-Block des repo-lokalen Konventionsdokuments trägt **1673** Zeilen, davon **824** in
einem einzigen Eintrag — mehr als die übrigen **achtzehn zusammen** (801). Am 2026-07-28 waren
es **47**; von den 803 Zeilen, um die die Datei seither gewachsen ist, entfallen **777** auf
diesen einen Eintrag. Der Eintrag ist die Feldtabelle der Span-Erfassung samt ihren erklärten
Abweichungen.
...
§Source precedence führen je sieben Ränge, keiner davon ist er) — eine bindende technische
Festlegung lag damit in einem ungerangten Dokument. Zweitens ist sein Gegenstand ein anderer:
er registriert **Abweichungen von der adoptierten Baseline**. Die Feldtabelle weicht von nichts
ab; sie *ist* die Festlegung.
...
undeklariertes Spec-Dokument *„nicht normativ zitierbar"*. Damit ist der Zielort aus
Folgepflicht 1 nicht mehr alternativlos, sondern die zweitbeste von zwei Möglichkeiten.
...
[ADR-0011](0011-telemetrie-erfassung-policy.md) Folgepflicht 1 wechselt vom Adaptions-Block ins
Technik-Stratum; die Vorgängerin bleibt im Übrigen **Accepted** und unangetastet. Drei
Festlegungen:
...
1. **Der Zielort ist `spec/spezifikation.md`.** Die Feldtabelle der Span-Erfassung (Feld ·
   ...
   **Begründung** der Folgepflicht bleibt tragend und wird durch den Wechsel erfüllt, nicht
...
3. **Die Aufwärts-Regel gilt ab der ersten Zeile.** Der bindende Text des Stratums trägt
   **keine** Entscheidungs- und keine Planungs-Kennung; Provenienz lebt allein in seiner
   ...
   Doku-Gate hält die Regel (Fitness Function unten).
...
- **Negativ, und das ist der Preis:** eine ab *Accepted* immutable ADR trägt jetzt eine
  Folgepflicht, die nicht mehr gilt, und **darf es nicht sagen**. Wer
...
|---|---|---|
| d-check (`matrix`, Klasse `spec-straten`) | Eine **verlinkte** Entscheidungs-Kennung im bindenden Text von `spec/spezifikation.md` (außerhalb der Historie) meldet `matrix-forbidden` | `make docs-check` |
| d-check (`ids`) | Eine **nackte** Entscheidungs-Kennung dort meldet `id-unlinked` — die zweite Hälfte derselben Kante | `make docs-check` |

**Beide Zeilen rot gesehen, mit Gegenprobe** (2026-08-01, gepinntes Image, `--network none`):
eine Zeile `siehe ADR-0011` in §3 meldet `spec/spezifikation.md:47 ADR-0011 id-unlinked`;
dieselbe Zeile als Markdown-Link meldet
`spec/spezifikation.md:47 ../docs/plan/adr/0011-telemetrie-erfassung-policy.md matrix-forbidden`.
Die Gegenprobe zeigt, dass die **Klassen-Mitgliedschaft** die Ursache ist und nicht ein
Nebeneffekt: derselbe Link bleibt still, sobald `spec/spezifikation.md` aus `spec-straten`
entfernt wird.
...
für die zwei Zeilen oben: der Mutations-Treiber kennt heute keine `docs-check`-Fehlerform, ihr
...
|---|---|---|
| 2026-08-01 | **Proposed** | Architect-Verdikt zur Einführung des Technik-Stratums; Anlass war ein Befund am Adaptions-Block (ein Eintrag größer als die übrigen achtzehn zusammen) und die zwei Entscheidungen mit `Schärft: —` |
