...
- **Folgepflicht 5 (der annehmende Lauf):** Fünf Auflagen binden den Übergang nach `Accepted`;
  **keine** hat einen Wächter — kein Modul des Doku-Gates liest sie
  ...
  selbstbezügliche Aussage** über den Bestand dieser Datei — mit Zahl wie ohne. (3) **Kein
  Markdown-Link in den Planning-Lifecycle und keiner nach `docs/reviews`:** Beide Bäume bewegt der
  Prozess — den Lifecycle der `git mv` des Zustandswechsels, die Review-Reports die Archivierung
  ihrer Welle —, und [`AGENTS.md`](../../../AGENTS.md) §3.11 verlangt dafür die **Kennung** statt
  der Pfad-Adresse. (4) **Kein Link-Ziel mit `)` darin und kein über einen Zeilenumbruch gesetzter
  Link:** Beides ließe die drei Zählungen auseinanderfallen, deren **Gleichheit** die
  Sonden-Aussage trägt. (5) **Keine rohe Link-Kopf-Sequenz in einem Code-Span:** Sie verfälscht
  beide Sonden und ist gate-unsichtbar. Dazu, außerhalb dieser Datei: Der ADR-Index
  ([`README.md`](README.md)) führt `Proposed` und wird beim Übergang nachgezogen — nach
