...
| 2026-09-06 | Überarbeitet, weiter **Proposed** | **Kein Review-Befund — zweiter Posten der Accept-Vorprüfung, und keine Runde konnte ihn fangen:** er schnappt erst mit dem Übergang selbst zu. Die Datei trug **selbstbezügliche** Messwerte — Zahlen, deren Gegenstand die Datei ist, in der sie stehen: die Stellenzahl der Beleg-Form, die drei Link-Zählungen, die Mächtigkeit beider Ziel-Mengen und die der tag-gepinnten Nennungen. Ihre **letzte** Änderung vor dem Einfrieren ist die `Accepted`-Zeile, und die schreibt derselbe Lauf, der sie nach [`AGENTS.md`](../../../AGENTS.md) §3.4 unerreichbar macht — eine hier eingefrorene Zahl wäre in genau diesem Moment falsch. **Gemessen, nicht vermutet:** hängt man je eine der `Accepted`-Zeilen von [ADR-0028](0028-anweisungssatz-gehoert-der-ausfuehrenden-rolle.md) und [ADR-0036](0036-ziel-fassung-regiert-den-sprung-v600.md) an eine Kopie dieser Datei, bewegen sich **alle drei** Link-Zählungen und **beide** Ziel-Mengen; die zwei übrigen bleiben stehen. Eine link-freie `Accepted`-Zeile ist konstruierbar — der Bestand führt solche (`grep -h '^\| .*\*\*Accepted\*\*' docs/plan/adr/[0-9]*.md \| grep -cvE '\]\('`) —, aber **keine Quelle verlangt sie**: [ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 3 (a) legt die **Beleg-Form** vor den Übergang, nicht Link-Freiheit, und `link-policy: always` macht jede Kennung, die die Zeile nennt, zum Link (`grep -n 'link-policy' .d-check.yml`). **Der Ausweg steht in der bindenden Quelle selbst**, und der Fall ist dort benannt: [`MR-025`](../../../harness/conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert) Setzung 2 führt *„die Vorkommen-Zahl in einer Datei, deren Kopf noch bearbeitet wird"* als mitwandernd und gibt ihr zwei Wege — *„entweder ausdrücklich als **kein** Erwartungswert gekennzeichnet oder durch ein Kriterium ersetzt, das den Gegenstand selbst misst"*. Der erste trägt nur, solange das Artefakt beschreibbar ist; §3.4 nimmt ihn mit dem Übergang weg. Also der zweite: **das Kommando bleibt, der eingefrorene Wert fällt** — bei der Beleg-Form, bei den drei Link-Zählungen, bei beiden Ziel-Mengen und bei den tag-gepinnten Nennungen, also auch dort, wo die Sonde nichts bewegte: welchen Wortlaut die `Accepted`-Zeile trägt, wählt der annehmende Lauf und nicht diese Zeile. **Die Erschöpfungs-Aussage verliert dabei nichts**, weil sie nie an einer Mächtigkeit hing: Die Einzelprüfung läuft über die **Menge**, die das Kommando ausgibt, und ihr Ergebnis steht über die Ziel-**Klassen** — Planning-Lifecycle und `docs/reviews/**` kommen nicht vor, die ADR-Geschwister liegen flach, die Register-Ablage ist ortsfest. Ebenso die Drei-Zählungen-Probe: tragend war immer ihre **Gleichheit**, nie ihr Wert. **Die Verwandtschaft zu [`AGENTS.md`](../../../AGENTS.md) §3.11 ist die Bauform, nicht der Gegenstand** — dort ist das Bewegliche ein fremdes Artefakt und der Träger eine Adresse, hier ist es diese Datei und der Träger eine Zahl; die Sektion wird dadurch nicht erweitert. **Ein Vollständigkeits-Anspruch entsteht hier nicht:** gezogen ist, was die Sonde und die Form-Frage erreicht haben; dass die Datei keine weitere selbstbezügliche Zahl trägt, sagt diese Zeile nicht. Außerhalb dieser Klasse ist kein Satz geändert. Der Statuswechsel bleibt offen: er braucht eine Runde mit tragendem Verdikt |
