...
- **Folgepflicht 5 (der annehmende Lauf):** Fünf Auflagen binden den Übergang nach `Accepted`;
  **keine** hat einen Wächter — kein Modul des Doku-Gates liest sie
  ...
  ([ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 3 a); eine Verletzung macht
  zusätzlich den Vorher/Nachher-Beleg der Geschichte-Tabelle unwahr. (2) **Keine neue
  selbstbezügliche Aussage** über den Bestand dieser Datei — mit Zahl wie ohne — **und keine
  Änderung, die eine stehende falsch macht**. Unter das zweite Verbot fällt namentlich das
  Urteil über die tag-gepinnten Baseline-Nennungen: Eine Nennung außerhalb eines
  ...
  und kein über einen Zeilenumbruch gesetzter Link:** Beides ließe die drei Zählungen
  auseinanderfallen, deren **Gleichheit** die Sonden-Aussage trägt. (5) **Keine rohe
...
| 2026-09-06 | Überarbeitet, weiter **Proposed** | **Kein Review-Befund — Accept-Vorprüfung.** Runde 7 (`2026-09-06-adr-0037-konsistenz-review-runde-7.md`) hat alles Übrige bestätigt; dieser Posten stammt aus Träger (a) von [ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 3, der die Beleg-Form **vor** den `Accepted`-Übergang legt — danach ist derselbe Satz durch [`AGENTS.md`](../../../AGENTS.md) §3.4 unerreichbar. Die Datei trug **einen** Markdown-Link in den tag-gepinnten vendored Baum: die Vorlagen-Nennung des Reconciliation-Registers. Er steht jetzt in der Form, die die Datei ohnehin führt — Tag genannt, Pfad baseline-relativ, Inline-Code statt Link ([ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 2: *„**Nicht** dazu gehören der lokale Präfix … und die Zeilennummer als alleiniger Locator"*) —, und ist damit kein Einzelfall, sondern ein weiteres Vorkommen der Form, die diese Datei durchgängig führt; **wie viele Stellen sie trägt, gibt das Kommando aus statt einer eingefrorenen Zahl** (`grep -cE 'Baseline .v6\.0\.0.,' docs/plan/adr/0037-*.md`). **Gezählt ist die Form, nicht die Beleg-Eigenschaft:** die neu hinzugekommene Stelle ist ein Vorlagen-Zeiger und trägt weder Abschnittsnamen noch Zitat; der Beleg der Aussage steht im Satz danach und trägt alle drei Teile. **Verloren geht dabei nicht allein die Navigierbarkeit.** Sie zählt nach jener Festlegung nicht zum Beleg, dessen drei Teile Tag, Datei- und Abschnittsname sowie Zitat sind. Mit ihr geht der **gate-sichtbare Wächter dieses einen Pfades**: Er wäre beim nächsten Sprung nicht verfallen, sondern **rot geworden** — genau das misst der Vorher/Nachher-Lauf weiter unten. Die Stelle tritt damit zur *stillen Hälfte* hinzu, ohne in deren Zahl mitzuzählen: ihr Pfad trägt den lokalen Präfix nicht, an dem jene Sonde ansetzt. **Gemessen ist die Klasse, nicht die Zeile.** Die Ziel-Menge der Markdown-Links steht hier als **Kommando statt als Zahl** — gebildet über den Ziel-**Pfad**, weil die Wander-Frage am Pfad hängt und nicht am Anker (`grep -oE '\]\([^)]+\)' <datei> \| sed -E 's/^\]\(//; s/\)$//; s/#.*$//' \| sort -u`). **Die zweite Lesart ist mitbenannt, damit die Formulierung nicht zwei Mengen trägt:** über die Ziel-**Zeichenkette** samt Anker gebildet ist es eine andere (`grep -oE '\]\([^)]+\)' <datei> \| sed -E 's/^\]\(//; s/\)$//' \| sort -u`). Über die **Pfad**-Menge läuft die Einzelprüfung, Ziel für Ziel, und **kein** Ziel darin bewegt der Prozess. **Getragen ist dieses Urteil von der Eigenschaft, nicht von einer Aufzählung der Ziel-Klassen:** [`AGENTS.md`](../../../AGENTS.md) §3.11 sagt es selbst — *„die Linie hängt an der Eigenschaft wandert auf Anweisung und nicht an der Aufzählung der Bäume"* — und verwirft damit den Katalog, den eine Klassen-Liste an dieser Stelle wieder aufmachte: Sie müsste vollständig sein, um zu tragen, und ein Baum, den sie nicht nennt, ließe sie still falsch werden. Gefragt wird deshalb je Ziel, ob der Prozess seinen Ort bewegt; dafür braucht die Prüfung weder die Mächtigkeit der Menge noch ihre Klassen. Wo die Antwort an einer eigenen Entscheidung hängt, steht sie dort — die Register-Ablage ist samt ihren Verzeichnissen ortsfest ([ADR-0034](0034-register-verzeichnis-form-und-die-ortsfestigkeit-der-register-datei.md) Festlegung 5). **Die Vollständigkeit des Musters ist geprüft, nicht geglaubt** — drei Zählungen müssen übereinstimmen: der rohe Link-Kopf (`grep -oE '[]][(]' <datei> \| wc -l`), das Ziel-Muster zeilenweise und dasselbe Muster über `tr '\n' ' '` umbruch-sicher (`grep -oE '\]\([^)]+\)' <datei> \| wc -l`). **Tragend ist ihre Gleichheit, nicht ihr Wert.** Ein Ziel mit `)` darin und ein über den Zeilenumbruch gesetzter Link ließen sie auseinanderfallen. **Zwei Instrument-Fallen sind dabei umgangen und gehören zur Aussage:** Beide Sonden sind so geschrieben, dass ihr eigener Abdruck sie nicht trifft — die Sequenz, die sie suchen, kommt in ihnen nicht vor —, und keine zählt mit `-c -o`, dessen Ergebnis zwischen GNU grep (Zeilen) und ugrep (Vorkommen) auseinandergeht; `grep -oE … \| wc -l` liefert unter beiden dieselbe Zahl. **Beide Richtungen gesehen** ([`AGENTS.md`](../../../AGENTS.md) §3.6): die Simulation des nächsten Re-Baselines — `v6.0.0` → `v6.1.0` auf der betroffenen Zeile, `make docs-check`, byte-exakt zurückgesetzt — meldete **vorher** genau **1** `target-missing` und **nachher 0**. **Die zweite Adress-Form bleibt stehen und ist benannt:** die tag-gepinnten Nennungen — **wie viele, gibt das Kommando aus statt einer eingefrorenen Zahl** (`sed -E 's/\]\([^)]*\)//g' <datei> \| grep -oE '\.harness/baseline/v[0-9]' \| wc -l`) — sind Operanden der Kommandos, die [`MR-025`](../../../harness/conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert) Setzung 1 verlangt, nicht Adressen eines Belegs; sie liegen außerhalb von `codepaths.roots` (`grep -n 'roots:' .d-check.yml`) und sind die *stille Hälfte*, die [ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 1 ausdrücklich stehen lässt. **Steering-Loop-Gehalt: die Klasse blieb über sieben Runden ungeprüft.** Alle sieben Reports führen [`AGENTS.md`](../../../AGENTS.md) §3.11 in ihrem Eingangs-Kontext, keiner nennt die Fundstelle (`grep -l 'reconciliation\.template\.md' docs/reviews/2026-09-06-adr-0037-konsistenz-review*.md \| wc -l` → **0**, kein Erwartungswert). Ihre Register-Zuordnung fällt bei der Slice-Closure, nicht hier ([`AGENTS.md`](../../../AGENTS.md) §3.10) |
| 2026-09-06 | Überarbeitet, weiter **Proposed** | Verengte Nachprüfung `2026-09-06-adr-0037-adress-form-verengte-nachpruefung.md` — **keine Runde 8**: Prüfgegenstand war der Nacharbeits-Commit der Zeile darüber, nicht die Datei. Bestätigt sind dort die Korrektur, die Klassen-Messung und beide Richtungen des Gegenbeispiels; gegengefahren ist die Messung mit einem **strengeren Instrument** (Code-Fences **und** Inline-Code gestrippt, bevor gezählt wird), und die Wander-Frage ist zusätzlich **am Werkzeug statt am Wortlaut** geprüft (`internal/archive/collect.go` und `harness/tools/slice-mv.sh` bewegen nichts in der Register-Ablage). Der eine blockierende Posten lag im **Protokoll**, nicht im Argument und nicht in der Wahrheit. **M-1:** Der Zähler der eindeutigen Ziele, auf dem die Erschöpfungs-Aussage der Zeile darüber ruht, stand **ohne das Kommando, das ihn ausgibt** — und dieselbe Formulierung trägt **zwei Lesarten mit zwei Werten**: gezählt über den Ziel-Pfad oder über die Ziel-Zeichenkette samt Anker. Beide Sonden stehen jetzt dort, die gemeinte ist benannt, und über sie läuft die Einzelprüfung; der ausgewiesene Wert ändert sich dadurch nicht — bestritten war das Instrument, nicht die Zahl ([`MR-025`](../../../harness/conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert) Setzung 1). **Warum das vor dem Übergang steht:** [ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 3 (a) legt die Beleg-Form vor die Annahme, und danach ist derselbe Satz durch [`AGENTS.md`](../../../AGENTS.md) §3.4 unerreichbar. **L-1 mitgezogen, obwohl der Report ihn als nicht blockierend ausweist:** Das Wort *Belege* trug weiter als sein Zähl-Merkmal — gezählt ist die **Form** der Nennung, und die neu hinzugekommene Stelle ist ein Vorlagen-Zeiger ohne Abschnittsnamen und ohne Zitat; die Zeile sagt das jetzt und nennt, wo der vollständige Beleg steht. **INFO-1 mitgezogen:** *„Verloren geht allein die Navigierbarkeit"* war enger als die Messung — mit dem Link ging der gate-sichtbare Wächter dieses einen Pfades, und die zweite Begründung (*„beim nächsten Sprung ohnehin verfallen"*) trägt für ihn nicht: er wäre nicht verfallen, sondern **rot geworden**. Die Zeile nennt jetzt beide Verluste und sagt, warum die Stelle zur dort ausgewiesenen *stillen Hälfte* hinzutritt, ohne in deren Zahl mitzuzählen. **Die Link-Zahl der Zeile darüber ist nach diesem Schreiben neu gefahren**, weil diese Zeile selbst Links hinzufügt; die drei Zählungen dort stimmen weiterhin überein, und die Ziel-Menge blieb dieselbe. **Ein neuer Vollständigkeits-Anspruch entsteht hier nicht**, und außerhalb dieser drei Posten ist kein Satz geändert. Der Zuschnitt ist derselbe wie bei Runde-6-M-1 — ein Zähler, dessen Instrument nicht ausgibt, was danebensteht; die Register-Zuordnung dieser Klasse fällt bei der Slice-Closure, nicht hier ([`AGENTS.md`](../../../AGENTS.md) §3.10). Der Statuswechsel bleibt offen: er braucht eine Runde mit tragendem Verdikt |
| 2026-09-06 | Überarbeitet, weiter **Proposed** | **Kein Review-Befund — zweiter Posten der Accept-Vorprüfung, und keine Runde konnte ihn fangen:** er schnappt erst mit dem Übergang selbst zu. Die Datei trug **selbstbezügliche** Aussagen — solche, deren **Subjekt die Datei ist**, in der sie stehen; ob ein Zahlwert dabeisteht, entscheidet nichts: die Stellenzahl der Beleg-Form, die drei Link-Zählungen, die Mächtigkeit beider Ziel-Mengen, die der tag-gepinnten Nennungen und ebenso jeder Satz, der **ohne** Zahl über den Bestand dieser Datei urteilt. **Die Trennlinie ist das Subjekt, nicht der Zahlwert.** Eine Aussage, deren Subjekt ein **benannter abgeschlossener Vorgang** ist — eine Review-Runde, ein Slice, eine Welle —, ist durch ihr Subjekt datiert und bleibt wahr, mit Zahl wie ohne; eine Aussage über **die Datei** ist es nicht, ebenfalls mit Zahl wie ohne. Ihre **letzte** Änderung vor dem Einfrieren ist die `Accepted`-Zeile, und die schreibt derselbe Lauf, der sie nach [`AGENTS.md`](../../../AGENTS.md) §3.4 unerreichbar macht — eine hier eingefrorene Aussage über den eigenen Bestand wäre in genau diesem Moment falsch, auch eine ohne Zahl. **Gemessen, nicht vermutet — und das Subjekt der Messung ist ein benannter Vorgang:** Die Bestätigungsprüfung `2026-09-06-adr-0037-accept-uebergang-selbstbezug-bestaetigung.md` hat den Übergang an Kopien simuliert, je einmal mit der `Accepted`-Zeile von [ADR-0028](0028-anweisungssatz-gehoert-der-ausfuehrenden-rolle.md) und der von [ADR-0036](0036-ziel-fassung-regiert-den-sprung-v600.md), und belegt, dass ein Teil der Sonden-Werte dabei wandert und ein Teil stillsteht. **Welcher Teil, steht hier nicht:** Das hängt am Wortlaut der `Accepted`-Zeile, den erst der annehmende Lauf wählt — eine Zeile, die eine Kennung verlinkt, welche die Ziel-Menge bereits führt, bewegt sie nicht. Eine link-freie `Accepted`-Zeile ist konstruierbar — der Bestand führt solche (`grep -h '^\| .*\*\*Accepted\*\*' docs/plan/adr/[0-9]*.md \| grep -cvE '\]\('`) —, aber **keine Quelle verlangt sie**: [ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 3 (a) legt die **Beleg-Form** vor den Übergang, nicht Link-Freiheit, und `link-policy: always` macht jede Kennung, für die `.d-check.yml` ein **Muster** führt, zum Link — nicht jede Kennung, die die Zeile nennt (`awk '/^ids:/,/^matrix:/' .d-check.yml \| grep -c 'link-policy: always'`). **Der Ausweg steht in der bindenden Quelle selbst**, und der Fall ist dort benannt: [`MR-025`](../../../harness/conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert) Setzung 2 führt *„die Vorkommen-Zahl in einer Datei, deren Kopf noch bearbeitet wird"* als mitwandernd und gibt ihr zwei Wege — *„entweder ausdrücklich als **kein** Erwartungswert gekennzeichnet oder durch ein Kriterium ersetzt, das den Gegenstand selbst misst"*. **Für eine Aussage über die Datei selbst** trägt der erste nur, solange das Artefakt beschreibbar ist; §3.4 nimmt ihn mit dem Übergang weg. Für einen **fremden** Gegenstand — die Evidence-Zahl des Beobachtungs-Registers, eine Messung an einer Plandatei, die Zählungen der Rumpf-Blöcke — bleibt er der Weg, den die Setzung gibt, und diese Zeile tastet ihn nicht an. Also der zweite: **das Kommando bleibt, der eingefrorene Wert fällt** — bei der Beleg-Form, bei den drei Link-Zählungen, bei beiden Ziel-Mengen und bei den tag-gepinnten Nennungen, also auch dort, wo die Sonde nichts bewegte: welchen Wortlaut die `Accepted`-Zeile trägt, wählt der annehmende Lauf und nicht diese Zeile. **Die Erschöpfungs-Aussage verliert dabei nichts**, weil sie nie an einer Mächtigkeit hing: Die Einzelprüfung läuft über die **Menge**, die das Kommando ausgibt, und ihr Urteil fällt je Ziel an der **Eigenschaft**, die [`AGENTS.md`](../../../AGENTS.md) §3.11 bindet — nicht an einer Aufzählung der Ziel-Klassen, die dieselbe Sektion verwirft. Ebenso die Drei-Zählungen-Probe: tragend war immer ihre **Gleichheit**, nie ihr Wert. **Die Verwandtschaft zu [`AGENTS.md`](../../../AGENTS.md) §3.11 ist die Bauform, nicht der Gegenstand** — dort ist das Bewegliche ein fremdes Artefakt und der Träger eine Adresse, hier ist es diese Datei und der Träger eine Aussage über sie selbst; die Sektion wird dadurch nicht erweitert. **Ein Vollständigkeits-Anspruch entsteht hier nicht:** gezogen ist, was die Sonde und die Form-Frage erreicht haben; dass die Datei keine weitere selbstbezügliche Aussage trägt, sagt diese Zeile nicht. Außerhalb dieser Klasse ist kein Satz geändert. Der Statuswechsel bleibt offen: er braucht eine Runde mit tragendem Verdikt |
| 2026-09-06 | Überarbeitet, weiter **Proposed** | Bestätigungsprüfung `2026-09-06-adr-0037-accept-uebergang-selbstbezug-bestaetigung.md` — **keine Runde 8**: Prüfgegenstand war der Nacharbeits-Commit der Zeile darüber und die Frage, ob der Übergang selbst eine Aussage der Datei umstößt. **Die Falle, um deretwillen jener Commit lief, ist entschärft:** Der simulierte Übergang machte in beiden Varianten keine Zahl der geprüften Fassung falsch, weil zu keiner ihrer Sonden noch ein eingefrorener Wert stand. Bestätigt sind dort außerdem die Fundmenge der selbstbezüglichen Kommandos, die Instrument-Freiheit beider Sonden und die Vorbedingung aus [ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 3 (a); gegen die **Entscheidung** steht auch in diesem Lauf kein Befund. Behoben sind die zwei blockierenden MEDIUM. **M-1:** Die Erschöpfungs-Aussage war als **Katalog der Ziel-Klassen** gebaut — genau die Bauform, die [`AGENTS.md`](../../../AGENTS.md) §3.11 verwirft; als Liste war sie zudem unvollständig, denn der Carveout-Lifecycle, den jene Sektion als **erste** der von ihr verallgemeinerten Entscheidungen führt, kam darin nicht vor, ebenso wenig die Einträge des Adaptions-Blocks und die Welle-Pläne. Sie steht jetzt auf der **Eigenschaft** — je Ziel gefragt, ob der Prozess seinen Ort bewegt — und braucht damit weder eine vollständige Klassen-Liste noch die Mächtigkeit der Menge; die Ziele, die unter keine der genannten Klassen fielen, sind mit demselben Kriterium beantwortet statt übergangen. Bestritten war die Trägerschaft, nicht das Ergebnis. **M-2:** Der Sweep war über **Zahlen** definiert und ließ die selbstbezügliche Aussage **ohne** Zahlwert durch — die Sonden-Aussage der Zeile darüber, die am eingefrorenen Stand anders ausfährt, sobald die `Accepted`-Zeile eine Kennung verlinkt, welche die Ziel-Menge bereits führt. Der Gegenstand ist jetzt über das **Subjekt** definiert: Eine Aussage über einen benannten abgeschlossenen Vorgang ist durch ihr Subjekt datiert, eine Aussage über die Datei ist es nicht — mit Zahl wie ohne. Nach dieser Linie ist die Sonden-Aussage gezogen; sie nennt den Vorgang, der sie gefahren hat, und lässt offen, welcher Teil der Werte wandert, weil das am Wortlaut der `Accepted`-Zeile hängt. **L-1 ist mitbehoben, obwohl der Report ihn als nicht blockierend ausweist:** Er trägt dieselbe Klasse wie M-1 — *„`link-policy: always` macht jede Kennung … zum Link"* reichte weiter als die Konfiguration, deren `link-policy` an benannten Mustern hängt; der Halbsatz nennt jetzt das Muster als Bedingung und trägt das Kommando, das sie zählt. **INFO-1 ist mitbehoben, weil die neue Definition ihn verlangt:** Der Satz über den ersten der zwei Wege aus [`MR-025`](../../../harness/conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert) Setzung 2 stand unqualifiziert neben den Stellen, die ihn für **fremde** Gegenstände weiter benutzen; er ist jetzt auf die Aussage über die Datei selbst eingeschränkt. **Die fünf Auflagen für den annehmenden Lauf stehen jetzt in der Datei** — als Folgepflicht 5 in §Konsequenzen, samt dem Nachzug des ADR-Index; keine von ihnen hat einen Wächter. **Ein neuer Vollständigkeits-Anspruch entsteht hier nicht**; geändert ist außerhalb der zwei Klassen kein Satz, hinzugekommen allein Folgepflicht 5. Der Statuswechsel bleibt offen: er braucht eine Runde mit tragendem Verdikt |
| 2026-09-06 | Überarbeitet, weiter **Proposed** | Zweite Bestätigungsprüfung `2026-09-06-adr-0037-accept-uebergang-zweite-bestaetigung.md` — **keine Runde 8**: Prüfgegenstand war der Nacharbeits-Commit der Zeile darüber und erneut die Frage, ob der Übergang selbst eine Aussage umstößt. Bestätigt sind dort beide behobenen MEDIUM samt erschöpfter Fundmenge und die fünf Auflagen als vollständig und in beiden Präzedenz-Varianten einhaltbar; gegen die **Entscheidung** steht auch in diesem Lauf kein Befund. **M-1 — und keine der bisherigen Simulationen konnte ihn fangen:** Sie liefen allein an dieser Datei; der Übergang aber schreibt nach Folgepflicht 5 **zwei** Dateien in **einem** Commit, und der Posten lag in deren Satz über die zweite. Er nannte den Statuswert des ADR-Index und verpflichtete denselben Commit, ihn zu ändern — eine Zustandsbehauptung, die den von ihr angeordneten Vorgang um null Commits überlebt. Sie ist **gestrichen**; die Pflicht steht ohne sie, und der Nachzug bleibt nach [ADR-0024](0024-derivatives-register-gehoert-der-rolle-seines-originals.md) im selben Architect-Commit. Der Wortlaut stammte aus dem Verdikt-Block der Vorrunde, wo er trägt: Ein Report ist durch sein eigenes Datum gebunden, ein einfrierendes Artefakt nicht. Es ist damit dieselbe Trennlinie wie eine Zeile darüber — Subjekt = benannter abgeschlossener Vorgang trägt, Subjekt = laufender Bestand nicht —, angewandt auf einen **fremden** Gegenstand. **L-1 mitgezogen, obwohl der Report ihn als nicht blockierend ausweist:** Auflage 3 stand als Katalog aus zwei Bäumen und damit in der Bauform, die [`AGENTS.md`](../../../AGENTS.md) §3.11 verwirft — dieselbe, die eine Zeile darüber gerade aus der Erschöpfungs-Aussage entfernt wurde. Sie fragt jetzt je Ziel nach der Eigenschaft *wandert auf Anweisung*; die Bäume, die der Katalog übersprang — die Carveout-Ablage, die jene Sektion als **erste** der von ihr verallgemeinerten Entscheidungen führt, und die Eintragsdateien des Adaptions-Blocks —, sind damit erfasst statt nachgetragen, und die Vorgänge, die daneben stehen, sind Beispiel und nicht Kriterium. Der Fall ist im Bestand realisiert und nicht konstruiert: Eingefrorene `Accepted`-Zeilen anderer ADRs tragen Markdown-Links in die Carveout-Ablage, deren Dateien der Prozess nach `done/` bewegt — **wie viele solcher Zeilen, gibt das Kommando aus statt einer eingefrorenen Zahl** (`grep -h '^\| .*\*\*Accepted\*\*' docs/plan/adr/[0-9]*.md \| grep -c 'carveouts/'`). **INFO-2 mitgezogen, weil er die Klasse von M-1 an einem eigenen Satz trägt:** Für das Urteil über die tag-gepinnten Baseline-Nennungen stand keine der Auflagen. Es bekommt keinen sechsten Katalog-Eintrag — das wäre die Bauform, die L-1 gerade gekostet hat —, sondern Auflage 2 trägt jetzt **beide** Richtungen: keine neue selbstbezügliche Aussage **und** keine Änderung, die eine stehende falsch macht. [ADR-0016](0016-verweis-traegt-tag-und-zitat.md) Festlegung 2 deckt den Fall unabhängig, indem sie den lokalen Präfix aus der Beleg-Form nimmt. **INFO-1 ist nicht aufgenommen:** Er gilt dem Beleg-Kommando einer Commit-Message und weist die zwei betroffenen Sätze selbst als wahr und stabil aus; am Text dieser Datei ist nichts zu ändern. **Der Übergang ist diesmal in beiden Hälften simuliert** — `Accepted`-Zeile **und** Index-Nachzug, je in den zwei Präzedenz-Varianten des Bestands. Gezogen ist, was diese Simulation und die zwei Befunde erreicht haben; ein Vollständigkeits-Anspruch über die Aussagen dieser Datei entsteht hier nicht. Außerhalb dieser drei Stellen ist kein Satz geändert. Der Statuswechsel bleibt offen: er braucht eine Runde mit tragendem Verdikt |
