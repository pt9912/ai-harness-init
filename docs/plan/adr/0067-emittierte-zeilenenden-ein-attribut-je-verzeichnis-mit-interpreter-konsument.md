...
**3. Die Klasse folgt dem Boden — und die Herkunft ist je Zeile benannt.** *Abgeleitet aus der Tabelle* heißt: ihre Zeile führt das Verzeichnis selbst oder seine Unterbäume als „reine tool-erzeugte Infrastruktur" und nennt in diesem Verzeichnis keine Adopter-Fläche (das Prinzip dort: konvergent ist die Ausnahme für tool-eigene Infrastruktur, die der Adopter nicht editieren soll). *Zweifelsregel* heißt: die Tabelle nennt eine Adopter-Fläche oder lässt die Zeile ungewogen, und im Zweifel gilt skip-if-present.

| Ziel-Datei | Klasse | Herkunft der Klasse |
|---|---|---|
| `.harness/.gitattributes` | **konvergent** | **abgeleitet aus der Tabelle** — die Unterbäume `.harness/baseline/<tag>/` und `.harness/skills/*` stehen dort als konvergente tool-Infrastruktur, und der Nachbar `.harness/.gitignore` (ebenfalls ein von `git` fixierter Name, im selben Verzeichnis) ist konvergent (Code-Aufzählung in `internal/emit/enforce.go`); eine Adopter-Fläche nennt die Tabelle in diesem Verzeichnis nicht |
...
| F — **nichts tun** | keine Änderung an Code, Text oder Vertrag | jedes Ziel, das ein Windows-Entwickler mit Voreinstellung klont, trägt CRLF in Skripten, die `bash` nicht lesen kann, und in einem vendored Baum, dessen Prüfsummen alle brechen; der Ausfall ist **laut**, nicht still — der Guard endet bei jedem Aufruf mit Exit 2 (Kontext), der git-eigene Träger mit 127, der vendored Baum fällt durch die Prüfsumme; ob Claude Code den Exit 2 des Guards als blockierend liest, ist ungemessen, und liest es ihn nicht so, ist der Guard wirkungslos — laut, aber ohne Halt |
| G — **vier Verzeichnisse** (`harness/mk/` aus der Menge streichen; das Kriterium „ein Konsument bricht" allein) | das Kriterium trägt ohne Vorsorge-Zweig: für make-Fragmente ist kein Bruch gemessen (GNU Make 4.3 verträgt CR, Probe in Festlegung 1; `make baseline-verify` läuft mit CRLF in `Makefile`, `d-check.mk` und den Fragmenten durch); eine skip-if-present-Datei und ihre Meldung weniger; die Vorsorge erreicht den Fall ohnehin nur teilweise — sie endet an der Wurzel, solange dort `Makefile` CRLF trägt und ein make es zuerst liest | die Vorsorge gegen ein make, das CR nicht verträgt, und gegen andere Konsumenten der Fragmente entfällt (ungemessen, nicht ausgeschlossen); `harness/mk/` ist eine Adopter-Fläche (`vorgaben.mk`, eigene Fragmente), deren Dateien im autocrlf-Klon ohne die Zeile CRLF tragen | <!-- d-check:ignore (der Pfad entsteht erst im gebootstrappten Ziel) -->

**Zu G:** Die Entscheidung bleibt bei fünf Verzeichnissen — die Zeile kostet eine Datei und eine Meldung, und sie hält die Fragmente an derselben Stelle wie die Skripte, deren Rezepte sie aufrufen. Wer den Vorsorge-Zweig nicht will, streicht `harness/mk/` aus der Tabelle in Festlegung 1 und aus der Klassen-Tabelle in Festlegung 3, ohne dass eine andere Festlegung sich ändert. <!-- d-check:ignore (der Pfad entsteht erst im gebootstrappten Ziel) -->

## Konsequenzen

- **Positiv:** Ein Klon mit `core.autocrlf=true` bekommt die Skripte, Fragmente, Hooks und den vendored Baum mit LF, soweit die Datei aus Festlegung 3 an ihrem Pfad liegt und mit LF im Index steht; `baseline-verify` und der git-eigene Träger laufen dort über ihre Shebang-Zeile bzw. gegen ihre Prüfsummen. Eine Adopter-Wurzel kann das für diese Dateien nicht aufheben (nächstes Verzeichnis gewinnt); bei belegtem skip-if-present-Pfad entscheidet die Datei des Adopters (Festlegung 5).
...
| `go test` (**Test zu bauen**) | **Meldungsinhalt je skip-if-present-Pfad:** an einem belegten Pfad (fremder Inhalt ohne `eol=lf`) nennt die Meldung des Laufs den Pfad **und** die Aussage aus Festlegung 4, was dann gilt (die Zeile fehlt, die Dateien des Verzeichnisses tragen im Klon mit `core.autocrlf=true` CRLF); rot gesehen durch eine Meldung, die nur den Pfad nennt | `make test` |
...
| `make mutate` (**Fall zu bauen**) | ein Eintrag der Aufzählung entfällt, oder die Zeile trägt `eol=crlf` → der Test der ersten Zeile färbt rot; die Meldung verliert die Aussage, was dann gilt → der Test der Meldungs-Zeile färbt rot | `make mutate` |
...
**Träger der Trigger:** der erste hängt am Runner-Ereignis der Grenze in [`LH-QA-04`](../../../spec/lastenheft.md#lh-qa-04--plattform-matrix), der vierte an der ersten Fitness-Zeile. Für den **zweiten und den dritten gibt es keinen Wächter**: keine Stichprobe gebauter Ziele wird gezogen, keine Messung an einem Windows-`make` genommen. Der Trigger-Audit der Closure prüft jede ADR gegen ihren Trigger und ist damit ein vorhandener Träger, sieht aber eine Messung nicht, die keiner nimmt. **Benannt, nicht geschlossen** — beide sind bis dahin eine Absichtserklärung, kein bewachter Trigger.
