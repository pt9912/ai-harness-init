...
Constraint"*. Die Schleife war von innen nicht aufzuschneiden; der Eintritt war eine
Auftraggeber-Entscheidung der Klasse, die
...
Wer sich später auf diesen Fall beruft, hat zu zeigen, dass die Senkung die Rolle blockierte, die
sie hätte entscheiden müssen.
...
Eingabe-Vertrags**. Zwei Enden, beide am Werkzeug gemessen:

- **2026-07-29 war der Vordergrund herstellbar.** `tool_input` trug über vier echte Aufrufe die
  Schlüssel `subagent_type`, `prompt`, `description` **und** `run_in_background`; ein Rollen-Typ
  mit `true` wurde abgelehnt, derselbe Typ mit `false` lief unmittelbar davor durch — und die
  `usage`-Zähler lagen an diesem Tag in der `tool_response` eines Vordergrund-Aufrufs
  (`docs/reviews/2026-08-02-span-schema-messreihen.md` §1, §2 und §3).
- **2026-08-15 ist er es nicht mehr.** Das Eingabe-Schema von `Agent` führt `prompt`,
  `description`, `subagent_type`, `model` und `isolation` — **kein** `run_in_background` — und
  lässt keine zusätzlichen Felder zu; die Probe fiel am Betriebsart-Zweig (Messung §1, §2).
...
Versehen des Aufrufers. Der Beleg für die Änderung selbst sind die zwei Payload-Messungen, nicht
die Doku.

**Die Grenze dieser Beleglage, benannt statt geglättet.** Gemessen ist, dass das Modell den
Schalter nicht mehr **sendet** und dass sein Schema ihn nicht mehr **führt**. Eine Sonde, die ihn
trotzdem sendet und die Ablehnung des Werkzeugs beobachtet, ist **nicht** gefahren. Für den Guard
macht das keinen Unterschied — er sieht die Payload, und in ihr fehlt das Feld. Für Alternative D
unten macht es den ganzen: sie setzt das Feld **nach** dem Modell ein und umgeht dessen Schema.
...
- **Der `Agent`-Span trägt von den neun erfassten Werten genau einen:** `model_version`. Kein
  ...
  die des Laufs.
- **Der zugehörige Subagenten-Strom trägt die Rolle in jeder Zeile:** `agent_type` und
  `agent_role` stehen schon im `SubagentStart` (zweimal `architect`, einmal `general-purpose`) und
  in jedem Span des Laufs — im Lauf, in dem diese ADR entsteht, in **allen** seinen Spans zum
...
genau diesen einen Abnehmer; ohne ihn stünde ein Zweig da, dessen beide Ausgänge `exit 0` sind.
...
`usage`-Zähler, hält er; trägt er sie nicht, hält er nicht.** Der Preis gehört vorher benannt,
weil er die Verstetigung mitbestimmt: die Doku nennt zwei Kombinationen und erklärt das Feld für
`"defer"` als ignoriert — `"allow"` überspringt für **jeden** Agenten-Aufruf das
Permission-System, `"ask"` fragt bei jedem nach. Beides ist eine Entscheidung über die
Durchsetzung und nicht nebenbei zu treffen; die Messung selbst ist davon nicht betroffen, ihre
Verstetigung sehr wohl.
...
- **Positiv:** die sechs Rollen sind wieder startbar, und die Rollen-Achse der Telemetrie trägt
  weiter — gemessen, nicht gehofft (oben).
...
- **Folgepflicht 2 — der Zeiger auf den Carveout.** Modul 7 verlangt, dass die Stelle, an der die
  ...
  *„Gate-Konfiguration zeigt per `# CO-<NNN>`-Kommentar auf den Carveout — sonst ist die
  ...
  der Kopf des Guards und Abweichung 5 im Spec-Stratum. Beide tragen den Zeiger **noch nicht** —
  das ist Implementer- und Spec-Arbeit, keine dieser ADR.
- **Folgepflicht 3 — der Folge-Slice zur Messung aus Festlegung 4, geschnitten vom Planner.** Ohne
  ...
  ADR"*) und in eine Folge-ADR zu überführen. Diese ADR benennt die Bedingung; sie schreibt den
  Slice nicht.
...
|---|---|---|
| bats — `test/agent-guard.bats`, Fall *„guard: JEDER Typ in .claude/agents/ laeuft durch"* | Kein Typ mit einer Datei in `.claude/agents/` wird abgewiesen; der Fall fährt über **jede** Datei des Verzeichnisses, eine neue Rolle ist damit automatisch gedeckt | `make test` (in `make gates`) |
...
nicht, dass ein Rollen-Lauf am Ende Zähler trägt. Für Festlegung 3 gibt es keinen Wächter, und
das ist eine Aussage, kein Auslassen: solange die Zähler in keiner Payload stehen, gibt es kein
Gegenbeispiel, das rot werden könnte ([`AGENTS.md`](../../../AGENTS.md) §3.6). An seine Stelle
tritt das `Letzte Prüfung`-Datum von [CO-002](../carveouts/CO-002-token-achse-je-rolle.md) und
das Carveout-Audit je Welle — Beobachtung durch Wiedervorlage, nicht durch Sensor.
...
|---|---|---|
| 2026-08-15 | **Proposed** | Architect-Auftrag zur bereits vollzogenen Senkung `83cf01d`. Grundlage ist die Messung in [`docs/reviews/2026-08-15-agent-guard-tool-vertrag.md`](../../reviews/2026-08-15-agent-guard-tool-vertrag.md); der Eintritt in die blockierte Schleife war eine Auftraggeber-Entscheidung ([`MR-015`](../../../harness/conventions.md#mr-015--change-request-bei-personalunion-von-auftraggeber-und-entwickler)-Klasse) |
