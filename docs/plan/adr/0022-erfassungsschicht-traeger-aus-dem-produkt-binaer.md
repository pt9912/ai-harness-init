...
| **liegt vor**        | das Produkt-Release als **Multi-Plattform-Archiv**: alle Plattform-Binaries in **einem** Asset, der Bootstrap wählt das für das Ziel passende aus | Bootstrap | tragfähig, und es löst Annahme (a) auf — **Alternative H** |
...
**Warum G und nicht H — die Abwägung, an der die Annahme hängt.** Alternative H trägt alle
Plattform-Binaries in **einem** Asset und **wählt** darum die Plattform des Ziels, statt sie vom
Host zu unterstellen; sie braucht Annahme (a) nicht. Ihre Mechanik ist keine neue: das Produkt holt
und entpackt heute schon ein Zip-Asset mit sha256-Pin vor dem Entpacken
(`internal/fetch/baseline.go`, `grep -n 'archive/zip' internal/fetch/baseline.go` → eine Zeile),
und das Archiv liest die Standardbibliothek, nicht `unzip`. Was H kostet, liegt **nicht** in der
Mechanik, sondern in der **Vertriebsform**: der Release-Lauf lädt heute die nackten Binaries der
...
Adopter sechs Binaries für eines laden, in einer Größe, die hier **ungemessen** ist.

**Die Wahl fällt auf G, solange die Annahme hält, und der Preis dafür steht in ihr:** kein Fall
ist bekannt, in dem Bootstrap-Host und Hook-Plattform auseinanderfallen, und für den Fall, dass
einer auftritt, ist H der benannte Ausgang — nicht als Notlösung, sondern als der Weg, der die
Frage strukturell nicht stellt. **Wer G für zu schmal hält, greift Annahme (a) an, nicht die
...
| **H — das Produkt-Release als Multi-Plattform-Archiv**: ein Asset trägt alle Plattform-Binaries, der Bootstrap wählt das für das Ziel passende aus und legt es ab | **löst Annahme (a) auf** — liegen alle Plattform-Binaries bei, wird die Plattform des Ziels **gewählt** statt vom Host unterstellt; die Mechanik existiert im Produkt und ist gehärtet: `internal/fetch/baseline.go` holt und entpackt heute schon ein Zip-Asset, mit sha256-Pin **vor** dem Entpacken, Größenschranke davor und typisierten Fehlern, die [`LH-QA-02`](../../../spec/lastenheft.md#lh-qa-02--reproduzierbarkeit) und [`LH-QA-03`](../../../spec/lastenheft.md#lh-qa-03--minimale-abhängigkeiten) im Text nennen; kein neuer Mechanismus und keine neue Abhängigkeit (das Archiv liest die Standardbibliothek, nicht `unzip`); kein committeter Platzhalter wie bei F, kein Bauschritt, keine zweite Plattform-Matrix — das Archiv **ist** sie | die **Release-Form ändert sich**: heute lädt der Release-Lauf sechs nackte Binaries hoch (`gh release upload "$tag" dist/*`) und der Start-Smoke prüft je Plattform genau eine solche Datei; ein Adopter lädt sechs Binaries für eines — die Archivgröße ist **ungemessen**, ebenso die Auswahl-Mechanik auf einem Windows-Ziel; und der abgelegte Träger trägt die Bootstrap-Fähigkeit mit sich wie bei G |
...
| **J — `docker run` gegen ein gepinntes Image je Hook-Aufruf** | kein Artefakt im Ziel; der Träger altert nicht, weil er nicht liegt | die Schwelle aus [ADR-0011](0011-telemetrie-erfassung-policy.md) bindet dann **je Aufruf** statt einmalig, und ein Container-Start je Tool-Call liegt erkennbar darüber; dazu ein Netz- oder Cache-Zustand im Hook-Pfad, den [`LH-FA-01`](../../../spec/lastenheft.md#lh-fa-01--repo-bootstrappen) nicht zusagt |
...
- **Wenn ein Bootstrap für eine andere Plattform ausgeführt wird als die, auf der die Hooks
  ...
  entschied, ist damit gegenstandslos. Die Plattform-Frage ist neu zu stellen, und der Weg, der
  sie ohne die Annahme beantwortet, ist **H**; D und E bleiben daneben verfügbar, tragen aber
  jeweils ihren eigenen Einwand aus der Alternativen-Tabelle.
