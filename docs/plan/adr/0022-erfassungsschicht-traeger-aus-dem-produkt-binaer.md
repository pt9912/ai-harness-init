...
| **I — ein Binär committet im Zielrepo** | zeitlos verfügbar, auch im frischen Klon — genau die Lücke, die G, F und H offen lassen | bricht die Setzung, mit der [ADR-0011](0011-telemetrie-erfassung-policy.md) Festlegung 3 den Bestand aus dem versionierten Baum hält (*„Spans liegen außerhalb des versionierten Baums, und das ist Korrektheit"* — ein Artefakt im Arbeitsbaum ginge in den Gate-Hash ein), und bindet ein Doku-Repo an **eine** Plattform; ein Binär im versionierten Baum ist zudem kein Artefakt, das ein Re-Lauf konvergent nachziehen kann |
| **J — `docker run` gegen ein gepinntes Image je Hook-Aufruf** | kein Artefakt im Ziel; der Träger altert nicht, weil er nicht liegt | die Schwelle aus [ADR-0011](0011-telemetrie-erfassung-policy.md) bindet dann **je Aufruf** statt einmalig, dass ein Container-Start je Tool-Call darüber liegt, ist plausibel und hier **ungemessen** — wie der Aufschlag von G, den Folgepflicht 9 schuldet; dazu ein Netz- oder Cache-Zustand im Hook-Pfad, gegen den [`LH-QA-03`](../../../spec/lastenheft.md#lh-qa-03--minimale-abhängigkeiten) das Ziel netzlos hält |
...
| 2026-08-23 | **Accepted** | Angenommen nach acht Review-Runden. Tragend geändert wurde dabei zweierlei: der Grund für die Wahl — er ist der **Preis** gegenüber der sonst gleichwertigen Einbettungs-Variante, kein Beweis und keine Einzigkeit —, und die Abzählung der Wege, die um vier Mitglieder wuchs, nachdem der Auftraggeber das Produkt-Release als Bündel benannt hatte. Der Weg, der die Plattform-Frage stellbar macht, statt sie zu unterstellen, steht seither als **Alternative H** und als Ausgang des Triggers zu Annahme (a). Zwei Ergänzungen der kommenden Regelwerks-Fassung sind vorweggenommen statt abgewartet |
