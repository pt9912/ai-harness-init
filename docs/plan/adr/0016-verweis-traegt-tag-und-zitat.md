...
Nennungen. [ADR-0015](0015-rollen-eigentum-an-norm-artefakten.md) trägt **keine** — für ein
Artefakt, das vor seiner Annahme noch geschrieben wird, gilt Festlegung 3 statt des Cutoffs.
...
inhaltlich überschrieben."*) und **keine Regel über die Form eines Verweises** — die eine
Nennung von *Verweis* dort gilt der Supersede-Provenance; `grundlagen-referenz-richtung.md` führt
...
|---|---|---|
| Werkzeugname `check-references` → **Referenz-Richtungs-Gate** | 4 Regelwerk-Dateien + das ADR-Template | keine — ein Name weicht der Eigenschaft, dieselbe Bewegung, die Festlegung 2 verlangt. Kostet dieses Repo nichts: außerhalb der Prosa, die über die Umbenennung schreibt, kennt das Repo den Namen nicht — kein Aufruf, kein Gate-Name, kein Target (`git grep -l 'check-references' -- ':!.harness/baseline' ':!docs'` → leer) |
...
| E — [ADR-0013](0013-technik-stratum-als-zielort.md) mit einer Folge-ADR superseden | formal die von §3.4 vorgesehene Bahn | die Entscheidung hat sich nicht geändert; ein Supersede für einen kaputten Pfad ist ADR-Inflation und macht aus einem Doku-Defekt einen Entscheidungs-Vorgang. Der Preis ist zudem groß: **22** Verweis-Vorkommen aus **11** lebenden Dateien zeigen auf ADR-0013 (Link-Form, Baseline und Zeitdokumente ausgenommen; das Kommando steht in [ADR-0017](0017-doku-gate-ausnahme-fuer-ein-eingefrorenes-adr.md) §Konsequenzen — die Zahl wächst mit jedem neuen Verweis), und `matrix.status` verbietet Verweise auf superseded ADRs |
...
| 2026-08-09 | **Accepted** | Annahme durch den Auftraggeber nach der Bestätigungsrunde `docs/reviews/2026-08-09-adr-0015-0016-0017-bestaetigungsrunde.md` (Kern bestätigt, beide blockierenden Befunde behoben); ab hier immutabel ([`AGENTS.md`](../../../AGENTS.md) §3.4) — spätere Schärfungen als neue ADR mit *Supersedes*. **Festlegung 3 (a) am eigenen Übergang vollzogen:** kein Beleg dieser ADR trägt einen lokalen Baseline-Pfad (`git grep -oE '\]\([^)]*\.harness/baseline/[^)]*\)' -- 'docs/plan/adr/001[567]*.md'` → keine Ausgabe); die zwei verbliebenen Nennungen benennen das Verzeichnis als **Gegenstand der Sonde**, nicht als Adresse eines Belegs. Vor dem Wechsel gezogen, weil §3.4 sie danach einfriert: das `check-references`-Kommando gibt aus, was es misst; Option E trägt die gemessene Zahl (**22** aus **11** lebenden Dateien statt 16 aus 10); *„kein Wort über Verweise"* ist auf die Verweis-**Form** verengt, weil `modul-04-adrs.md` *Verweis* einmal nennt — für die Supersede-Provenance; und die Status-Angabe zu [ADR-0015](0015-rollen-eigentum-an-norm-artefakten.md) steht nicht mehr im Fließtext, weil derselbe Lauf sie weiterdreht |
