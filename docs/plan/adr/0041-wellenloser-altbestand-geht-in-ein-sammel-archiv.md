...
n=0; for r in docs/reviews/*.md; do rb="${r##*/}"; \
  ...
  [ "$h" -gt 0 ] && n=$((n+1)); done; echo "$n von $(ls docs/reviews/*.md|wc -l)"   # 153 von 349, kein Erwartungswert
```
...
| E — Werkzeug ändern, Altbestand flach lassen: eine erklärte Untergrenze ohne Archiv | Nimmt C den Zwangs-Charakter, ohne ein Byte des Altbestands zu bewegen: Die Archivierung würde für künftige Wellen benutzbar, und für die 57 entstünde kein Verweis-Nachzug in ein eingefrorenes Artefakt | **Verworfen am Begriff, nicht am Preis** — und der Preis ist nicht der von B und mehr: Von den vier Ausgängen, die Folgepflicht 1 für B nennt, berührt E allein `untergrenze`, denn sie archiviert weiter unter Welle-Kennungen, und die tragen Ergebnisnotiz und Welle-Plan (gemessen, §Kontext Punkt 2); dafür muss diese Untergrenze zugleich die Einsammel-Regel binden — `Bestand.Slices()` führt heute jeden flach liegenden wellenlosen Slice mit, und ohne die zweite Wirkung zöge die nächste Welle-Closure den Altbestand doch in ihr Archiv. Der Begriff aber fehlt: eine Untergrenze, die kein Archiv ist, führt die adoptierte Baseline `v6.5.0` an derselben Stelle nicht — es wäre eine zweite Definition derselben Grenze, mit demselben Fehler wie der Datums-Schnitt in Festlegung 3. Sie löst die offene Vorbedingung nicht: `[haenger]` feuert auch für `welle-01` und ist keine Eigenschaft des Altbestands. Und die 57 blieben dauerhaft in der Form, die die Quelle für geschlossene, eingesammelte Vorgänge nicht vorsieht |
...
| 2026-09-12 | **Accepted** | Vollzogen in der Architect-Rolle. **Der Acceptance-Trigger ist eingelöst**, und der Beleg, den er verlangt, ist die **Reviewer-Runde vom 2026-09-12 zu ADR-0041, Runde 3** — gefahren in frischem Kontext gegen [ADR-0033](0033-wellen-archivierung-als-unterkommando.md), [ADR-0030](0030-eingefrorene-adresse-auf-den-planning-lifecycle.md) und die zitierte Stelle der adoptierten Baseline `v6.5.0`; ihre Kategorie-Summary nennt **kein HIGH und kein MEDIUM**, ihr Report liegt damit ohne blockierenden Befund in `docs/reviews/`. Dass es die **dritte** Runde ist, fordert [ADR-0040](0040-accept-uebergang-nennt-den-beleg-seines-triggers.md) Festlegung 2: Die Runden 1 und 2 meldeten blockierende Befunde, Beleg ist darum eine erneute Runde derselben prüfenden Rolle und nicht die Nachmessung des Kontexts, der sie auflöste. **Der eine LOW jener Runde ist vor diesem Umschlag behoben** — dieselbe Handlung trägt in §Verglichene Alternativen und in Folgepflicht 1 jetzt dieselbe Zahl; Festlegung 2 deckt das, denn sie verlangt eine weitere Runde nur nach einem **blockierenden** Befund. Wer eine ADR annimmt, sagt keine Quelle dieses Repos — gemessen in [ADR-0018](0018-ziel-fassung-regiert-die-migration.md) §Geschichte, hier nicht gedoppelt. **Ab hier bindet [`AGENTS.md`](../../../AGENTS.md) §3.4:** Korrekturen entstehen als Folge-ADR mit `Supersedes`. |
