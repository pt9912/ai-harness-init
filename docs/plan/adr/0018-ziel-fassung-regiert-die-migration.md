...
Schritte auf `v5.9.0` und `v5.12.0` deckt der Delta-Nachweis darunter). **Die zwei Trefferzahlen
sind von Hand über die Fundstellen gezählt; kein Kommando dieses Dokuments gibt genau sie aus**
([`MR-025`](../../../harness/conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert)
Setzung 1) — tragend ist die **Lektüre** der Treffer, nicht ihr Betrag; reproduzierbar mit
Kommando sind die zwei Delta-Nachweise darunter. Die einzigen Aussagen über Normativität
...
→ **1**), über **+13/−4** Zeilen
(`git diff --shortstat v5.3.1 v5.9.0 -- lab/regelwerk/modul-02-harness-bootstrap.md`). Der
Abschnitt heißt unverändert §Freshness-Audit der vendored
...
| 2026-08-28 | **Accepted** | **Entscheidung des Auftraggebers vom 2026-08-28, vollzogen in der Architect-Rolle.** Wer eine ADR annimmt, sagt **keine** Quelle dieses Repos und keine der vendored Baseline: abgesucht sind die gerankten Quellen, [`AGENTS.md`](../../../AGENTS.md), der Harness-Einstieg, dieser Index und der vendored Baum, mit vier Begriffen um den annehmenden Akt (`git grep -cniE -e 'annehmend' -e 'Accept-Akteur' -e 'nimmt die ADR an' -e 'setzt den Status' -- spec AGENTS.md CLAUDE.md harness README.md docs/plan/adr/README.md .harness/baseline` → **eine** Datei, **3** Zeilen). Alle drei stehen in [`MR-015`](../../../harness/conventions.md#mr-015--change-request-bei-personalunion-von-auftraggeber-und-entwickler), dessen Geltungsbereich ausdrücklich `spec/lastenheft.md` ist; auf die ADR-Statuszeile überträgt er nicht. **Grenze:** ein Negativ aus vier aufgezählten Zeichenketten. Was der Record zeigt, ist der **Vollzug** unter dem Label der schreibenden Rolle — `2046fda`, `1b540c8`, `325411c` (`git log --oneline --grep='^Rolle Architect' -S'**Status:** Accepted' -- 'docs/plan/adr/0*.md'` → drei Zeilen; dieselbe Abfrage ohne `--grep` liefert **19** Zeilen, die drei sind darin die jüngsten; beide Zahlen wandern mit jeder weiteren Annahme, [`MR-025`](../../../harness/conventions.md#mr-025--eine-zahl-im-text-steht-neben-dem-kommando-das-sie-liefert) Setzung 2). Die Einwilligung dahinter zeigt `git` nicht. **Ab hier bindet [`AGENTS.md`](../../../AGENTS.md) §3.4:** die drei Zeilen darüber sind der letzte Fall, in dem eine Bewegung des Zielstands in dieser Datei nachgezogen werden durfte — jede weitere ist eine Folge-ADR mit `Supersedes`, und dieselbe Bahn gilt dem dritten Re-Evaluierungs-Trigger. **Vor dem Umschlag geprüft:** die abgedruckten Kommandos sind gefahren und liefern ihre Werte unverändert; zwei Zahlen haben ihre Mess-Basis nachgetragen bekommen — die zwei Trefferzahlen der Suche vom 2026-08-09 sind als Handzählung ausgewiesen, `+13/−4` steht neben dem Kommando, das genau sie ausgibt |
