...
- **Folgepflicht (Adaptions-Durchgang):** Die
  ...
  Auflösungs-Trigger oder ein anderer der fünf Ausgänge, mit Beleg.
...
- **Wenn die Ziel-Fassung während der laufenden Welle upstream wandert** *(der Zielstand ist
  beweglich; Auslöser ist ein gemessener Defekt im Zielstand, nicht ein neuer Tag)*: dann ist zu
  prüfen, ob die geänderte Prozedur die schon gelaufenen Durchgänge noch trägt — sonst verliert
  die Welle rückwirkend ihre Messlatte.
