...
**Slices:** slice-044 (Generator-Kompositions-Seam) **[done](../done/slice-044-generator-kompositions-seam.md)** (2026-07-24; Review KONFORM, Verifikation DoD BESTÄTIGT, `flat` byte-identisch); **nächster:** slice-045a ([hexSlice-Layout + Go-Renderer](slice-045a-hexslice-go-renderer.md), **in-progress**) → slice-045b ([`--arch`-CLI](../open/slice-045b-arch-cli.md)) → slice-046 (konditionaler a-check-Emitter, in §4 der Welle). slice-045 wurde bei [`ADR-0009`](../../adr/0009-hexslice-arch-realisierung.md) in **045a/045b** re-sliced (Layout+Renderer vs. CLI je eigene DoD).
