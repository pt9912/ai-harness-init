...
**Slices:** slice-044 (Generator-Kompositions-Seam) **[done](../done/slice-044-generator-kompositions-seam.md)** (2026-07-24; Review KONFORM, Verifikation DoD BESTÄTIGT, `flat` byte-identisch); **nächster:** slice-045 (`hexagonal`-Layout + Go-Renderer + `--arch`), dann slice-046 (konditionaler a-check-Emitter) — beide in §4 der Welle (per `cp` bei Schnitt).
