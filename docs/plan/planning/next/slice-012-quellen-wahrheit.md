...
|---|---|---|
| `AGENTS.md:15`, `harness/conventions.md:18` | „Betriebsregelwerk der adoptierten Baseline" → `raw.githubusercontent.com/…/main/kurs/de/agents-regelwerk.md` | **404** — die Monolith-Datei existiert upstream nicht mehr; v3.0.0 referenziert sie nirgends |
| `AGENTS.md:33-34` | „Skelett-Vorlagen der Baseline als ZIP" → `releases/latest/download/lab-templates.zip` | **404** — v3.0.0 liefert nur noch *ein* login-freies Asset (`lab-regelwerk.zip`) |
...
Ziel ist der Kurs unter `/kurs/de/` (Tag `v3.0.0`, verifiziert HTTP 200); die
präsente Form ist die vendored Baseline aus slice-011.

Dazu bekommt `harness/conventions.md` §Baseline den fehlenden **Adoptions-Bezugspunkt**:
die Stand-Zeile des Regelwerks („Kurs-Welle 25 · 2026-07-16", `regelwerk/README.md:3`)
und den Tag `v3.0.0` — heute steht dort nur „Templates: templates-v4".

**Abgrenzung.** Die Absätze, die die *Cache-/Vendoring-Mechanik* beschreiben,
gehören zu slice-011. Der inhaltliche Konformitäts-Nachzug ist slice-013. Der
„wortgleich"-Satz in der Begründung von [`MR-006`](../../../../harness/conventions.md#mr-006--regelwerk-cache-als-split-modul-verzeichnis) wird **nicht** angefasst — er
war für v1.2.0 zutreffend und steht nach slice-011 als **Historie**; historische
MR-Einträge werden nicht umgeschrieben.

## 2. Definition of Done

- [ ] `AGENTS.md` §1 und `harness/conventions.md` §Adoptierte Konventions-Quellen
      nennen eine **erreichbare** kanonische Quelle (Kurs `/kurs/de/`, auf `v3.0.0`
...
- [ ] `AGENTS.md` §1: der `lab-templates.zip`-Verweis (404) entfällt — die
      ...
      zweiten Asset.
- [ ] `harness/conventions.md` §Baseline trägt Stand („Kurs-Welle 25 ·
      2026-07-16") und Tag (`v3.0.0`); die Zeile „Templates: templates-v4" ist
...
- **`main` vs. Tag.** Die alte URL zeigte auf `main` (floating) — das stand schon
  ...
  ausdrücklich Tag- oder Digest-Pinning verlangt. Der Ersatz wird auf `v3.0.0`
  gepinnt; das Template stützt das („für harte Reproduzierbarkeit das Asset eines
  Tags ziehen statt `latest`").
