...
- **Ein Delta-Katalog findet eine Deckung nicht, die ein Volltext-Durchgang fände** — die Klasse
  ...
  Einzelfall ist gebunden — [slice-157](../open/slice-157-adaptions-durchgang-v5180.md) trägt die
...
| 6 | `grundlagen-traceability.md` | vier Zusätze: (a) Herkunfts-Anker nach dem Archivieren ist zweistufig (Stub → Archiv) · (b) Gegenrichtung der Ruheort-Regel für `MR-<NNN>`: der `git mv` zieht die Pfad-Berichtigung nach · (c) **zwei Rot-Quellen**: Baseline-Verweise tragen die Version, ein Versions-Sensor prüft jeden Pin gegen **eine** Deklaration · (d) „Beide Commits gehören in denselben Push" | D | (a) → [slice-158](../open/slice-158-archivierungs-schritt.md) · (b) → [slice-157](../open/slice-157-adaptions-durchgang-v5180.md) · (c) → [slice-162](../open/slice-162-versions-sensor-baseline-pins.md) · (d) → [slice-157](../open/slice-157-adaptions-durchgang-v5180.md), dort als **Übergabe an den Architect** (Hard-Rule-Nachbarschaft zu [`AGENTS.md`](../../../../AGENTS.md) §3.3, kein `MR`) |
