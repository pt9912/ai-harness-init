...
[slice-087](../done/slice-087-emittierte-doku-tische-init-invariant.md), den Beleg danach `slice-063`;
...
- [x] **(1) Eine ADR trägt die vier Werte der Tool-Spalte — Accepted, mit dem Trichter-Ausgang je
  ...
  *emittiert* nach [welle-09](../welle-09-modul-15-konformitaet.md) §3 verdienen kann — er endet
  ...
  von [slice-087](../done/slice-087-emittierte-doku-tische-init-invariant.md) — sie steht hier, weil sie
...
| **Implementer** ([slice-087](../done/slice-087-emittierte-doku-tische-init-invariant.md), dann `slice-063`) | zuerst die emit-seitige Neutralisierung **jedes** emittierten Dokuments, das ein nicht Init-invariantes `make`-Ziel behauptet, samt Wächter über den Dokument-Satz, danach die emittierte Doc-Gate-Konfiguration und die zwei Beleg-Richtungen in [`harness/tools/full-smoke.sh`](../../../../harness/tools/full-smoke.sh) | berührt **weder** ADR **noch** Lastenheft; die Reihenfolge ist keine Vorliebe, sondern die Bedingung aus DoD (1) |
...
- [`LH-FA-03`](../../../../spec/lastenheft.md#lh-fa-03--doc-gate-baseline-emittieren-f6-f7)
  **wird ebenfalls nicht bewegt — sie deckt den Träger schon.** Sie sagt `.d-check.yml` +
  ...
  [`MR-001`](../../../../harness/conventions.md#mr-001--doc-gate-schärfung-matrix--link-pflicht--anker-ids)
  — dieselbe Einordnung, die [slice-073](../in-progress/slice-073-emittierte-doc-gate-module.md) §3 für seine
...
- **Bericht oder Gate ist entschieden; die Messung dahinter bleibt trotzdem ein Befund.**
  ...
  **Init-invariant sind zwei** — `gates` und `help`. **Sieben nicht**, und sie
  ...
  Aufzählung von Fundorten ([slice-087](../done/slice-087-emittierte-doku-tische-init-invariant.md) §1).
  [`MR-017`](../../../../harness/conventions.md#mr-017--default-regel-für-emittierte-prüfbereiche-fail-closed)
...
- **Der Beleg für Block 4 verlangt beide Richtungen, und keine davon liefert dieser Slice.**
  ...
  **Dazu kommt eine Varianten-Klammer:** der Beleg ist in **beiden** Bootstrap-Varianten zu
  ...
  ([slice-087](../done/slice-087-emittierte-doku-tische-init-invariant.md) §6, am Voll-Smoke gemessen).
- **Die Behauptungen sind ein eigener Schnitt — und er ist Mitglied dieser Welle, nicht
  ...
  liegt deshalb als
  [slice-087](../done/slice-087-emittierte-doku-tische-init-invariant.md) in `open/` und **in** dieser
  ...
  [slice-073](../in-progress/slice-073-emittierte-doc-gate-module.md) DoD (3) ihn schon abnähme** — das trägt
...
- **Nicht in diesem Slice:** der Beleg selbst (`slice-063`), die Repo-Seite von Block 4
  ...
  ([slice-073](../in-progress/slice-073-emittierte-doc-gate-module.md) — sie nimmt `targets` ausdrücklich
...
[slice-086](../done/slice-086-vordergrund-per-updatedinput.md). Wer den Liefergegenstand dieses
...
[slice-082](../done/slice-082-adaptions-durchgang.md), dessen §3 zwei Norm-Artefakte in die
