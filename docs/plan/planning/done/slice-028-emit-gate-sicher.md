...
- [ ] [`LH-QA-01`](../../../../spec/lastenheft.md#lh-qa-01--keine-halluzinierten-gates-f4-f5-f6): der emittierte `docs-check` meldet **0 Befunde out-of-the-box** — belegt via `make smoke` (die heute 3 Befunde weg). Der **Voll-E2E-`make gates`-Beweis** ist [slice-024](slice-024-voll-smoke.md), hier **nicht** behauptet.
...
DoD vollständig + Review konform + Closure-Notiz → nach `done/`. **Entsperrt [slice-024](slice-024-voll-smoke.md)**
...
Vendored Baseline unberührt (alles emit-seitig). **Entsperrt [slice-024](slice-024-voll-smoke.md)**
