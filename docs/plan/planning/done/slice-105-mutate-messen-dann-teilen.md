...
([slice-117](../done/slice-117-lauf-ohne-ende-faerbt-rot.md) §3 nimmt sie mit).
...
| **`QUEUE_LOCK_TRIES` trägt keinen Zahn** (B-8, erste Hälfte). `grep -rln 'QUEUE_LOCK_TRIES' test/ \| wc -l` → **0** | **aufgeschoben**, zusammen mit der neuen Schranke — wer die Zeitschranken anfasst, fasst beide an | [slice-117](../done/slice-117-lauf-ohne-ende-faerbt-rot.md) DoD (3) |
