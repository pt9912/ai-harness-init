...
- **Offen und nicht in diesem Slice:** ob der zusammengelegte Einstiegspunkt die Schwelle aus
  ...
  [slice-095](../in-progress/slice-095-hook-aufschlag-gemessen.md); fällt die Messung negativ aus, ist die Antwort
...
- **Ob der zusammengelegte Einstiegspunkt die Schwelle aus
  ...
  [slice-095](../in-progress/slice-095-hook-aufschlag-gemessen.md); fällt die Messung negativ aus, wird
...
| Der Positions-Kommentar am Unterkommando-Zweig in `cmd/ai-harness-init/main.go` nennt Fall 154 als Wächter; der bewacht das Routing, nicht die Position | **[slice-095](../in-progress/slice-095-hook-aufschlag-gemessen.md)**, als Kommentar-Nachzug nach [`AGENTS.md`](../../../../AGENTS.md) §3.7 — er fährt genau diesen Einstiegspunkt. Der Satz ist **neu** mit diesem Slice und damit gebunden, kein Bestand. Die Position ist nicht bewachbar; sie gehört als Grenze benannt, wie `harness/tools/span-check.sh` es zweimal vormacht |
| Der Kopf von `span-report` im [`Makefile`](../../../../Makefile) sagt *„fuer einen Bericht soll niemand einen Container starten muessen"*, während das Rezept über `host-bin` einen startet — der Satz gilt dem Ziel, nicht dem Dogfood | **[slice-095](../in-progress/slice-095-hook-aufschlag-gemessen.md)**, derselbe Nachzug |
...
[slice-095](../in-progress/slice-095-hook-aufschlag-gemessen.md) negativ ausfällt — dann ist die Antwort
