...
| **8 (b)** — §Und das Fragment mountet | Keine Nachforderung an [`MR-010`](../../../../harness/conventions.md#mr-010): Der neue Absatz erlaubt die Mount-Form *„solange das Werkzeug nur liest"* und verlangt eine `MR` allein im Zweig *Fragment nicht einbinden*. Das Repo bindet ein und mountet read-only (`grep -c ':/repo:ro' d-check.mk` → **11**, `grep -c 'CURDIR):/repo"' d-check.mk` → **0**). Die Mount-Achse selbst trägt [slice-160](../open/slice-160-docker-form-hermetisch-und-beleg.md) |
