...
| # | Aussage | Belegklasse |
|---|---|---|
| 1 | `PreToolUse` feuert für `Agent`, und `tool_input` trägt `subagent_type` schon **vor** dem Lauf | **gemessen** — [slice-060](../done/slice-060-rollen-achse.md) §3 Zeile 8, an einer echten Sonde mit `"matcher": "Agent"`. `run_in_background` lag am 2026-07-29 in derselben Payload. **Für dieses Feld stehen heute drei Aussagen nebeneinander, und sie fallen auseinander:** vom Aufrufer **gesendet** wird es angenommen (2026-08-15 gemessen) · **wirksam** ist es nicht, der so gestartete Lauf läuft dennoch im Hintergrund (2026-08-15 gemessen) · ob es **die Hook-Payload erreicht**, hat zuletzt der Betriebsart-Zweig des Guards gelesen (2026-08-10: `ABSENT`), und diese Lesung ist heute nicht wiederholbar, weil der Zweig mit [`ADR-0019`](../../adr/0019-agent-guard-prueft-die-aufrufform.md) Festlegung 1 gefallen ist. Für die Feld-Tabelle unten heißt das: die dritte Aussage ist **offen**, dieses Protokoll ist ihr einziger Abnehmer ([`ADR-0019`](../../adr/0019-agent-guard-prueft-die-aufrufform.md) Folgepflicht 5), und was es aufzeichnet, ist die Beobachtung — nicht ein Ausfall des Hooks |
...
| 5 | Das dokumentierte Eingabe-Schema von `Agent` führt `prompt`, `description`, `subagent_type`, `model` — **kein** `run_in_background` | **gelesen — und für die Frage nach dem Schalter ohne Beweiskraft.** Die vendored Tabelle führt dieselben vier Felder schon seit dem Vendoring, also auch am 2026-07-29, als der Schalter nachweislich in der Payload lag und wirkte ([`ADR-0019`](../../adr/0019-agent-guard-prueft-die-aufrufform.md) §Kontext, dort nachgemessen); sie kann eine Vertragsänderung weder belegen noch widerlegen, und ihre Übereinstimmung mit einer Beobachtung ist keine Bestätigung. Belegen kann sie den **Standard**: ein weggelassener Schalter erzeugt einen Hintergrund-Lauf. Für diesen Slice folgt daraus: die Betriebsart trägt das Protokoll **beobachtend** — sie ist seit [`ADR-0019`](../../adr/0019-agent-guard-prueft-die-aufrufform.md) Festlegung 1 keine Vorbedingung einer Entscheidung mehr, und was in der Payload wirklich steht, sagt keine Tabelle, sondern erst die Zeile |
...
| `run_in_background` | *In welcher Betriebsart?* — die Betriebsart ist der Grund, aus dem eine Antwort ohne Zähler zurückkommt. **Welchen der drei Werte die Zeile trägt, weiß heute niemand, und genau deshalb bleibt das Feld:** gesendet wird der Schalter angenommen und bleibt wirkungslos (2026-08-15), die letzte Lesung an der Hook-Payload stammt vom 2026-08-10, und ihr Abnehmer ist gefallen. Steht in einer Zeile `true` oder `false`, ist belegt, dass der Wert den Hook **erreicht** — die eine Hälfte des ersten Re-Evaluierungs-Triggers von [`ADR-0019`](../../adr/0019-agent-guard-prueft-die-aufrufform.md), dessen andere Hälfte ein Lauf **mit** Zählern ist; steht dort `ABSENT`, ist die Lesung von 2026-08-10 wiederholt statt geerbt | genau drei Werte: `true`, `false`, `ABSENT` |
