...
**(1) Die Probe aus [slice-137](slice-137-beobachtungs-register-bekommt-seinen-ort.md), zwei
Konjunkte.** Das erste — *stammt der Befund aus dem Re-Baseline-Delta?* — lautet **nein**, und
...
[slice-070](slice-070-comment-claims-pruefbereich.md), das dieselbe Prüfung mit demselben Ergebnis
führt: leer und erkennbar statt gefüllt und falsch.
...
ist gelebt, nicht hypothetisch: `git grep -c 'make -k gates' -- docs/plan/` → **drei** Dateien
(`CO-004…:1`, `slice-081…:1`, `slice-133…:4`), und in der roten Phase seit
...
→ **0**. Zweitens liegt der `Makefile` überhaupt nicht im Prüfbereich (`sed -n '141p' Makefile`
zeigt die vier Pfad-Muster des Rezepts). Ob der Prüfbereich enger ist, als die Vollständigkeits-
Zeile klingt, ist eine eigene Frage mit eigenem Träger:
[slice-070](slice-070-comment-claims-pruefbereich.md).
...
- [ ] **(2) Jede Stelle, die die Zusage trägt, sagt, was der Code hält — und die Wege, die offen
      ...
      `git grep -inE 'gruenen (Gates|Checks)|grünen (Gates|Checks)' -- ':!docs/reviews' ':!docs/plan/planning/done' ':!.harness/baseline'`
      → heute **4** Fundorte, davon **2** im Dogfood (`Makefile`, `harness/tools/record-gates.sh`)
      und **2** in den emittierten Vorlagen. **Gebunden sind die zwei im Dogfood.** Jeder der drei
...
- [ ] Beobachtungs-Register fortgeschrieben — **falls es zum
      ...
      [slice-137](slice-137-beobachtungs-register-bekommt-seinen-ort.md), und die Reihenfolge der
      beiden ist frei. Existiert es nicht, wird genau das in §7 notiert, statt das Item stumm zu
      lassen.
...
[slice-130](slice-130-emitter-entscheidet-jedes-neue-template.md), und das ist kein Vorzug,
...
1. **Der Befund feuert in genau der Phase, in der das Repo steht.** Solange ein Gate rot ist,
   ...
   belegt: `git grep -c 'make -k gates' -- docs/plan/` → drei Dateien, alle aus der Zeit seit
...
[slice-137](slice-137-beobachtungs-register-bekommt-seinen-ort.md). Läuft dieser Slice nach jenem,
wird die Sichtung nachgeholt und ihr Ergebnis in §7 notiert.
