...
| **(3)** — drei Proben gegen den geltenden Vorlagensatz, Zusage trägt das Ergebnis | **erfüllt** | §2.1. Die drei Proben (templates.go, Block unter `StripCommentHints`) nennen `T=.harness/baseline/v6.9.0/templates`; der vierte Kommentar-Block (`unmaskQuotedCommentSyntax`) nennt denselben Tag. Der Tag ist **der aus `harness/conventions.md` §Baseline** (Zeile 11 trägt **Stand:** `v6.9.0`), nicht ein im Plan eingefrorener (`MR-033`). `git grep -c 'v6\.7\.2' -- internal/emit/templates.go` → **0** (Exit 1, keine Ausgabe). Jeder Operand löst auf (`[ -e ]`). Alle Proben wurden von diesem Lauf selbst gefahren: gegen `v6.9.0` **alle leer**, gegen den abgelösten `v6.7.2`-Baum **Fehler statt leer** (exit 2, „No such file or directory") — die Rot-Beschreibung aus DoD 3 ist damit unabhängig vom Reviewer-Report bestätigt. |
