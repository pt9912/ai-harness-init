...
# Probe-Werkzeug (awk) im PATH, faellt sie in den Abbruch. Der Operand haengt hinter den Aufruf
# ein `echo da`: damit traegt auch der unbekannte Ausgang den Marker, und `make doc-immutable`
# endet ueber einem d-check.mk ohne die Ziel-Definition mit Erfolg (Exit 0) — die Bindung wird
# dort gewaehlt, wo der laute Abbruch steht.
#
...
set -euo pipefail
sed -i "s@2>/dev/null),da)@2>/dev/null || echo da),da)@" internal/emit/emit.go
