...
#
# Nimmt dem Schreiber die KLEMME — das `defer clamp()` in spanEmit().
#
# Die Klemme ist tragend, nicht dekorativ: emitSpan() laesst jeden Fehlschlag als panic
...
set -euo pipefail
sed -i '/^\tdefer clamp()$/d' cmd/ai-harness-init/span_emit.go
